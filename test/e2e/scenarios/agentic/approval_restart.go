...
// every loop-lane consumer and the tools consumer are quiescent before the
// kill, so no redelivery can seat either loop warm on the replacement ahead of
// the answer. The record's revision is re-read after the replacement to show
// nothing touched it. The answer therefore reaches the cold branch
// (settleApprovalResponseWithoutLoop): step 0 reads the record and finds no
// newer retained request, the gate matches, I4 holds, the loop is rebuilt from
// the retained request and response, and the answer is applied warm on the
// rebuilt loop.
...
const (
	...
	// loopLaneOwner and toolsLaneOwner are the durable-consumer name prefixes
	// of the two components whose deliveries could reach a parked loop
	// (agentic-loop: fmt.Sprintf("agentic-loop-%s", ...) in setupConsumer).
...
// approvalRestartLanes are the consumers whose deliveries can touch a parked
// loop. Each is found by owner and filter, never by a guessed name, and each
...
func (s *Scenario) verifyApprovalAcrossReplacement(
	...
	if err := s.verifyReplacementCounters(ctx, gatedSuccess); err != nil {
		return err
	}
...
func (s *Scenario) verifyReplacementCounters(ctx context.Context, gatedSuccess map[string]string) error {
...
func (s *Scenario) parkApprovalLoop(
	...
	task := newApprovalGatedTask(time.Now(), "approval-restart-"+label, approvalLoopOwner)
	...
	loop := &parkedApprovalLoop{label: label, task: task, gate: gate, parked: observed, answer: answer}
...
// deadline that could decide instead of the human still ahead. A replacement
// that rewrote the record, or a stage that arrives after a deadline, would be
// proving something other than the answer being applied.
func (s *Scenario) assertParkedAcrossReplacement(
	...
	for _, loop := range parked {
		...
		if err := deadlinesAhead(observed.entity, now); err != nil {
...
// deadlinesAhead refuses a parked loop whose loop deadline or approval deadline
// has already passed. Either one firing settles the loop without the human:
// the loop deadline fails it on its first delivery after a rebuild, and an
// approval deadline auto-rejects it. The error names the budget to raise rather
// than letting a later wait time out and blame the recovery.
func deadlinesAhead(entity agentic.LoopEntity, now time.Time) error {
	if !entity.TimeoutAt.IsZero() && !now.Before(entity.TimeoutAt) {
		return fmt.Errorf("the replacement gap outran the loop's own deadline (timeout_at %s, now %s); raise "+
			"agentic-loop.timeout in configs/agentic.json above the replacement window",
			entity.TimeoutAt.UTC().Format(time.RFC3339), now.UTC().Format(time.RFC3339))
	}
	gate := entity.PendingApproval
	if gate == nil || gate.Timeout <= 0 {
		return nil
	}
	if deadline := gate.RequestedAt.Add(gate.Timeout); !now.Before(deadline) {
		return fmt.Errorf("the replacement gap outran the approval deadline (%s, now %s); raise "+
			"agentic-loop.approval_timeout in configs/agentic.json above the replacement window",
			deadline.UTC().Format(time.RFC3339), now.UTC().Format(time.RFC3339))
	}
	return nil
}

// awaitAnswerApplied waits for the loop's agent.complete. A loop that settles
// on agent.failed instead was rebuilt and then failed — most likely on the
// deadline it inherited, or with continuation_unavailable — which is not the
// answer applied, and is reported as what it is.
func (s *Scenario) awaitAnswerApplied(ctx context.Context, agent jetstream.Stream, loop *parkedApprovalLoop) error {
	...
	if err := waitForStreamSubject(ctx, agent, completeSubject, 90*time.Second); err != nil {
		failed, failedErr := streamSubjectCount(ctx, agent, "agent.failed."+loop.task.LoopID)
		if failedErr == nil && failed > 0 {
			return fmt.Errorf("the replacement settled the %s loop on agent.failed, not agent.complete: the "+
				"answer was not applied: %w", loop.label, err)
		}
		return fmt.Errorf("the replacement did not carry the %s loop to a terminal after its answer: %w", loop.label, err)
	}
	return nil
}
...
// from the retained request and response. Exactly one tool message answers
// the gated call: the executed result for an approval, the human's rejection
// for a reject. A second one would be the gate's placeholder leaking beside
// the answer.
func checkAppliedAnswer(
	...
	var promptSeen, callSeen bool
	var answers []agentic.ChatMessage
	for _, msg := range request.Messages {
		switch msg.Role {
		case "user":
			promptSeen = promptSeen || strings.Contains(msg.Content, prompt)
		case "assistant":
			for _, call := range msg.ToolCalls {
				callSeen = callSeen || call.ID == gate.CallID
			}
		case "tool":
	...
	if !promptSeen {
		return fmt.Errorf("request %s does not carry the task prompt; the conversation was not rebuilt", wantRequestID)
	}
	if !callSeen {
		return fmt.Errorf("request %s carries no assistant turn calling %q; the conversation was not rebuilt",
			wantRequestID, gate.CallID)
	}
	if len(answers) != 1 {
...
// laneConsumer asks the server which consumer the lane's owner runs over the
// lane's subject root. Exactly one match is required.
func laneConsumer(ctx context.Context, js jetstream.JetStream, lane consumerLane) (jetstream.Consumer, error) {
	...
	if err := lister.Err(); err != nil {
		return nil, fmt.Errorf("list %s consumers: %w", lane.stream, err)
	}
	if len(matched) != 1 {
		return nil, fmt.Errorf("want exactly one %s* consumer on %s filtering %s, found %d: %v",
			lane.owner, lane.stream, lane.subjectRoot, len(matched), matched)
	}
	return stream.Consumer(ctx, matched[0])
}
