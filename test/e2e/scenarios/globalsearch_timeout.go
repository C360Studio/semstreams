...
// globalSearch calls on the semantic tier (the known-answer gate and the B0
// thematic eval). It exists for the HEAVY LOCAL qwen3-8b measurement
...
//
// UNSET (the CI / qwen3-1.7b default) leaves every call site on its own default,
// so default behaviour is byte-for-byte unchanged. Set it (e.g. "150s") only for
// the 8B run.
