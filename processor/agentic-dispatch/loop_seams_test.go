...
func newSeamTestComponent(t *testing.T) (*Component, *captureSink, *seamRecorder) {
	...
	// AutoContinue resolution now reads the shared loop view instead of a
	// process-local tracker, and these seam fixtures run no view. Every test
	// below is about refusal precedence, ownership, or publication, never about
	// auto-continue; a test that wants it opts in and supplies a view.
...
func TestApprovalIsNotOwnerScoped(t *testing.T) {
	...
	withPersistedLoops(c, map[string]*agentic.LoopEntity{seamTestLoopA: {
		...
		PendingApproval: &agentic.PendingApprovalState{
			CallID: "call-001", ExecutionID: approvalTestExecutionID, ToolName: "delete_rule",
		},
	}})
