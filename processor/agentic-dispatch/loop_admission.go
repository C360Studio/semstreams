...
type loopFacts struct {
	...
	ChannelType string
	ChannelID   string
	// Terminal is true when the durable record reports a settled state. There is
	// no second source to disagree with it any more: this layer deleted the
	// in-process tracker, so the fail-closed reading the two-source version
	// needed is now simply what the record says.
	Terminal bool
