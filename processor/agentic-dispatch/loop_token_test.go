...
func newLoopTokenTestComponent() (*Component, *captureSink) {
	...
	// AutoContinue now resolves through the shared loop view, which these token
	// fixtures do not run. The refusals under test happen at the submission
	// seam, before any continuation branch; the auto-continue test opts in.
