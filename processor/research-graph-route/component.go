...
func (c *Component) initRouter() error {
	...
	// Publish the paired client/router fields atomically. Stop cannot select
...
//
// The ctx passed in comes from the NATS subscription callback and remains live
// while Stop drains the exact subscription. Stop cancels the component context
// only after that native drain boundary completes or returns an error, then
// closes the LLM client after callbacks can no longer use it.
