...
func registerDecide(natsClient *natsclient.Client, platform component.PlatformMeta, logger *slog.Logger) {
	publisher := agentictools.NewNATSTriplePublisher(natsClient)
	executor := agentictools.NewDecideExecutor(publisher, platform)
	if err := registerGlobal(agentictools.DecideToolName, executor); err != nil {
