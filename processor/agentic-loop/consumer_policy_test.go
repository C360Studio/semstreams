	...
	for _, portName := range []string{"agent.task", "agent.response", "tool.result"} {
		for _, test := range []struct {
			...
			t.Run(portName+"/"+test.name, func(t *testing.T) {
				...
				err = c.setupConsumer(
					t.Context(), t.Context(), port, portName+".>", func(context.Context, []byte) error { return nil }, nil,
				)
				if test.wantErr {
					require.Error(t, err)
...
func TestLoopMaxDeliverCoversFixedBackOffBeforeConsumerAcquisition(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			err = c.setupConsumer(
				t.Context(), t.Context(), port, "agent.task.>", func(context.Context, []byte) error { return nil }, nil,
			)
			if test.wantErr {
				require.Error(t, err)
				require.True(t, errs.IsInvalid(err))
