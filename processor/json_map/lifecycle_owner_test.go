	...
	for index := range count {
		definition := component.PortDefinition{
			...
			Config: component.JetStreamPort{
				StreamName: fmt.Sprintf("S1_MAP_%d", index),
				Subjects:   []string{fmt.Sprintf("s1.map.%d", index)},
			},
		}
...
func TestLifecycleOwnerProductionStartStopOverlapRollsBackExactHandle(t *testing.T) {
	...
	owner := &Processor{
		name: "json-map-processor", logger: slog.Default(), natsClient: &natsclient.Client{},
		inputPorts: lifecycleJetStreamPorts(t, 2), metrics: &mapMetrics{},
...
func TestLifecycleOwnerProductionRunningStopKeepsCallbackAuthorityUntilClosed(t *testing.T) {
	...
	owner := &Processor{
		name: "json-map-processor", logger: slog.Default(), natsClient: &natsclient.Client{},
		inputPorts: lifecycleJetStreamPorts(t, 1), metrics: &mapMetrics{},
		waitForStreamInput: func(context.Context, string) error { return nil },
		consumeStream: func(
