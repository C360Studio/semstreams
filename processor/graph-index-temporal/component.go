...
// DeclarePorts is the component.PortDeclarer for graph-index-temporal: the ports
// CreateGraphIndexTemporal will report for rawConfig, computed without dependencies.
...
// CreateGraphIndexTemporal share.
func resolveConfig(rawConfig json.RawMessage) (Config, []component.Port, []component.Port, error) {
	var config Config
	if len(rawConfig) > 0 {
		if err := json.Unmarshal(rawConfig, &config); err != nil {
			return Config{}, nil, nil, errs.Wrap(err, "CreateGraphIndexTemporal", "factory", "config unmarshal")
		}
	} else {
	...
	if err := config.Validate(); err != nil {
		return Config{}, nil, nil, errs.Wrap(err, "CreateGraphIndexTemporal", "factory", "config validation")
	}
	inputs := make([]component.Port, len(config.Ports.Inputs))
	for index, definition := range config.Ports.Inputs {
		port, err := definition.Resolve(component.DirectionInput)
		if err != nil {
			return Config{}, nil, nil, errs.Wrap(err, "CreateGraphIndexTemporal", "factory", "resolve input port")
		}
		inputs[index] = port
	}
	...
	for index, definition := range config.Ports.Outputs {
		...
		if err != nil {
			return Config{}, nil, nil, errs.Wrap(err, "CreateGraphIndexTemporal", "factory", "resolve output port")
		}
		outputs[index] = port
	}
...
func (c *Component) Start(ctx context.Context) (startErr error) {
	...
	if err != nil {
		startErr := errs.Wrap(err, "Component", "Start", fmt.Sprintf("KV bucket creation: %s", graph.BucketTemporalIndex))
		if ctx.Err() != nil {
	...
	if err != nil {
		startErr := errs.Wrap(err, "Component", "Start", fmt.Sprintf("KV bucket creation: %s", graph.BucketTemporalIndexReverse))
		if ctx.Err() != nil {
			startErr = errs.Wrap(ctx.Err(), "Component", "Start", "context cancelled during reverse bucket creation")
		}
		return startErr
	}
