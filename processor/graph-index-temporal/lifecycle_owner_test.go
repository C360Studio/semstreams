...
func runLifecycleStartRollback(t *testing.T, blockCallback bool) {
	...
	c.subscribeForRequests = func(ctx context.Context, subject string, handler func(context.Context, []byte) ([]byte, error)) (*natsclient.Subscription, error) {
		sub, err := client.SubscribeForRequests(ctx, subject, func(callbackCtx context.Context, data []byte) ([]byte, error) {
			_ = c.Health()
			close(healthDone)
			if blockCallback {
				<-release
		...
		require.NoError(t, err)
		require.NoError(t, client.Publish(ctx, subject, nil))
	...
	if !blockCallback {
		...
		require.ErrorContains(t, startErr, "ENTITY_STATES")
...
func TestLifecycleOwnerDrainOrderSerializationAndRepeat(t *testing.T) {
	client := newLifecycleNATSClient(t)
	entered, release := make(chan struct{}), make(chan struct{})
	callbackCtx := make(chan context.Context, 1)
	runCtx, cancel := context.WithCancel(t.Context())
	sub, err := client.SubscribeForRequests(runCtx, "test.graph.temporal.lifecycle.order", func(ctx context.Context, _ []byte) ([]byte, error) {
		callbackCtx <- ctx
		close(entered)
		<-release
		return nil, nil
	...
	require.NoError(t, client.Publish(t.Context(), "test.graph.temporal.lifecycle.order", nil))
	require.NoError(t, client.GetConnection().Flush())
	<-entered
