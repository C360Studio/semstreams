...
func runLifecycleStartRollback(t *testing.T, blockCallback bool) {
	...
	require.NoError(t, err)
	_, err = graph.EnsureCatalogBucket(t.Context(), client, graph.BucketOutgoingIndex)
	require.NoError(t, err)
	_, err = graph.EnsureCatalogBucket(t.Context(), client, graph.BucketIncomingIndex)
	require.NoError(t, err)
...
func TestLifecycleOwnerDrainKeepsCallbackAndChildLiveUntilCompletion(t *testing.T) {
	...
	c := &Component{logger: lifecycleLogger(), initialized: true, lifecycleUsed: true, running: true, cancel: cancel, runtimeDone: done, llmClient: child, querySubscriptions: []*natsclient.Subscription{sub}}
	require.NoError(t, client.Publish(t.Context(), "test.graph.clustering.lifecycle.order", nil))
	require.NoError(t, client.GetConnection().Flush())
	<-entered
	...
	case <-child.closed:
	default:
		t.Fatal("LLM child not closed after callback drain and join")
	}
	require.NoError(t, c.Stop(t.Context()))
}
