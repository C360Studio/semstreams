...
func (*lifecycleEmbedder) Generate(context.Context, []string) ([][]float32, error) {
	return nil, nil
}
func (*lifecycleEmbedder) GenerateQuery(context.Context, []string) ([][]float32, error) {
	return nil, nil
}
func (*lifecycleEmbedder) Dimensions() int { return 1 }
func (*lifecycleEmbedder) Model() string   { return "lifecycle" }
func (e *lifecycleEmbedder) Close() error {
	e.once.Do(func() { close(e.closed) })
	return nil
}

var _ embedding.Embedder = (*lifecycleEmbedder)(nil)
...
func runLifecycleStartRollback(t *testing.T, blockCallback bool) {
	...
	_, err := graph.EnsureCatalogBucket(t.Context(), client, graph.BucketEntityStates)
	require.NoError(t, err)
...
func TestLifecycleOwnerDrainKeepsCallbackAndChildLiveUntilCompletion(t *testing.T) {
	...
	runCtx, cancel := context.WithCancel(t.Context())
	sub, err := client.SubscribeForRequests(runCtx, "test.graph.embedding.lifecycle.order", func(ctx context.Context, _ []byte) ([]byte, error) {
		callbackCtx <- ctx
		close(entered)
		<-release
		return nil, nil
	...
	child := &lifecycleEmbedder{closed: make(chan struct{})}
	c := &Component{logger: lifecycleLogger(), initialized: true, lifecycleUsed: true, running: true, cancel: cancel, runtimeDone: done, embedder: child, querySubscriptions: []*natsclient.Subscription{sub}}
	require.NoError(t, client.Publish(t.Context(), "test.graph.embedding.lifecycle.order", nil))
	require.NoError(t, client.GetConnection().Flush())
	<-entered
	...
	select {
	case <-child.closed:
		t.Fatal("embedder child closed before callback drain")
	default:
	}
	releaseCallback()
	require.NoError(t, <-stopResult)
	...
	case <-child.closed:
	default:
		t.Fatal("embedder child not closed after callback drain and join")
	}
	require.NoError(t, c.Stop(t.Context()))
}
...
func TestLifecycleOwnerTerminalDeadlineIsNotReplayed(t *testing.T) {
	...
	c := &Component{logger: lifecycleLogger(), initialized: true, lifecycleUsed: true, running: true, cancel: cancel, runtimeDone: done}
