	...
	for index := range count {
		definition := component.PortDefinition{
			...
			Config: component.JetStreamPort{
				StreamName: fmt.Sprintf("S1_GENERIC_%d", index),
				Subjects:   []string{fmt.Sprintf("s1.generic.%d", index)},
			},
		}
...
func TestLifecycleOwnerProductionStartStopOverlapRollsBackExactHandle(t *testing.T) {
	...
	owner := &Processor{
		name: "json-generic-processor", logger: slog.Default(), natsClient: &natsclient.Client{},
