...
between producers is PERMITTED, and nothing reports it.** The taxonomy vocabulary is shared, and overlap is
legitimate and sometimes intended. The boot-time composition rejection is retired, and so is the whole authorization
apparatus: `EntityDomainAuthority`, `NewEntityDomainAuthority` and `Authorize` are DELETED. With overlap permitted
there was nothing left to authorize, they had no production caller, and detecting a mis-chosen taxonomy token is a
*vocabulary* question — someone picked the wrong token — not one to answer at composition time. An offline
composition-validator report was implemented and then removed by the same ruling on the same day, as complexity
ratcheted for little gain; nothing regresses, because the rejection it replaced had never run.

`EntityDomainDelegation` is retained: the corpus audit AST-scans those literals for the registered set its
`domain_unregistered` rule consults, and that is its only consumer. Overlap collides nothing — `system` is position
3, so the IDs stay distinct, and ADR-099 level 0 is source x taxonomy, so the communities do too; the cross-source
pattern `org.platform.*.<domain>.*.*` returning every producer's entities in a taxonomy is the point. Exclusivity
may be reintroduced if a concrete use case appears.
...
**(SUPERSEDED 2026-08-28 — see the Supersession note above)**; O-6
...
4. **`domain` and `type` are delegated on the predicate-namespace pattern.** The framework reserves `agent`,
   ...
   `domain` or `domain.type` delegations in production Go. Two producers MAY declare one domain (superseding O-5,
   above), and nothing reports the overlap. An undelegated value in a builder or declaration is never a runtime
   ...
   > rejection. There is no longer an `Authorize` to call. Whether a boot-time act should exist for the undelegated
   > case is unruled and open.
