# GitHub #950 Slow-Consumer Attribution Complete Design Handoff

Baseline: `0f98936692a1c4910fe4353ee5db503a3f1393c4`

Phase: `pre-owner-design`

Body SHA-256: `68bd465a93b4c010f0d4d8a6d09228a52bd5dde107e32701f341a8062c782c8b`

Owner acceptance: approved for OpenSpec materialization and TDD implementation on 2026-08-12.
...
Keep the connection-wide ERROR as `{error}` only. This leaves the measured #950 failure unactionable while status and
health remain green.

### Option 1 — Extend the existing handler with log attribution

Keep `handleError` as the one owner. Attribute every subscription-bearing async error with the subscribed subject and
optional queue. For `ErrSlowConsumer`, also report the cumulative known drop count when nats.go supplies it.

Cost: logs remain the only signal. Subject and queue values are suitable for structured logs but not automatic
Prometheus labels.

### Option 2 — Add attribution and a dedicated metric

Add Option 1 plus a drop/error counter labelled by subscription identity.

Cost: no present dashboard, alert, or query consumer exists; deployment-authored subjects and queues can be
high-cardinality; existing `WithMetrics` is JetStream-only and absent from the primary binaries. This would create a
phantom, unwired surface.

### Option 3 — Add attribution and repair primary logger wiring

Add Option 1 and reorder composition so the client uses the later configured log counter/NATS-forwarding chain.

Cost: the full logger itself uses the connected NATS client, so this is a broader boot-order and feedback-loop design.
It is independently valid but unnecessary for putting attribution in the client's existing output.

## Recommendation and target behavior

Choose Option 1 only.

For every `handleError` invocation:

- Preserve level `ERROR`, message `NATS error`, and attribute `error`.
- With `sub == nil`, preserve the current generic shape and add no subscription fields.
- With `sub != nil`, add `subject=sub.Subject`; add `queue=sub.Queue` only when nonempty.
- Attribute every subscription-bearing async error. Restrict only the drop query—not identity—to
  `stderrors.Is(err, nats.ErrSlowConsumer)`.
- For a slow-consumer error, call `sub.Dropped()` once. On success emit the cumulative known `dropped` count. On
  failure omit `dropped` and emit `dropped_available=false`; do not log a second arbitrary diagnostic error.
- Do not query or emit `Pending`, `MaxPending`, or `PendingLimits`: these are racing post-event snapshots and are not
  needed to identify the affected subscription.
- Do not call `recordFailure` or alter circuit state, connection status, health, readiness, or callbacks.
- Add no metric, config, exported API, pending-limit control, or logger-wiring change.

The adopter's do-nothing path becomes sufficient: component authors configure and register nothing; an ordinary
framework ERROR identifies the subscription whenever nats.go supplies it.

## Binding owner rulings

| Ruling | Binding target |
|---|---|
| R1 — Attribution scope | Every async error with `sub != nil` gets subject attribution. |
| R2 — Drop scope | Only errors satisfying `stderrors.Is(err, nats.ErrSlowConsumer)` query `Dropped()`. |
| R3 — Fields | `subject` is always present for nonnil subscriptions; `queue` is omitted when empty. |
| R4 — Unavailable count | A failed `Dropped()` omits `dropped` and emits `dropped_available=false`. |
| R5 — No snapshot inference | Pending depth, high-water values, and limits are not read or logged. |
| R6 — Nil-sub path | Existing generic error-only shape is preserved. |
| R7 — Runtime semantics | Failure accounting, circuit state, connection status, health, readiness, and callbacks are unchanged. |
| R8 — Surface | No metric, exported symbol, config field, status field, or health field is added. |
| R9 — Scope boundaries | Pending-limit configuration remains on #586; primary logger composition is a separate issue. |
| R10 — Specification | Add the `nats-client-diagnostics` capability; no ADR is warranted. |
| R11 — Evidence | Unit plus real-NATS integration must drive the production handler with explicit synchronization. |
| R12 — E2E honesty | Coverage gap [#954](https://github.com/C360Studio/semstreams/issues/954) records that no product E2E stage proves this operator-visible behavior. |

## OpenSpec target

Change ID: `attribute-nats-subscription-errors`

Capability: `nats-client-diagnostics`. The existing `nats-streaming` purpose is explicitly publish-only, while
`graph-view-subscription` owns a distinct in-process fan-out class.

### Requirement: Subscription-bearing asynchronous NATS errors are attributable

The natsclient connection-wide asynchronous error handler SHALL emit an ERROR record preserving the original error.
When nats.go supplies a subscription, the record SHALL include its subscribed subject and SHALL include its queue
group only when nonempty.

For `nats.ErrSlowConsumer`, the handler SHALL report the subscription's cumulative known dropped-message count when
`Dropped()` succeeds. If that query fails, the record SHALL omit the count and report
`dropped_available=false`.

The handler SHALL NOT infer attribution from subjects, query pending depth or limits, record a connection failure, or
change connection status, health, readiness, or circuit state. A nil-subscription callback SHALL preserve the generic
error-only record.
...
- **THEN** failure count, circuit state, connection status, and health are unchanged

## TDD and evidence plan

1. Add `natsclient/client_async_error_test.go` to capture production `slog.Record` values and prove the nil-sub shape,
   ordinary subscription attribution, queue omission/presence, unavailable slow-consumer count, absent racing fields,
   and unchanged connection/circuit state.
2. Add `natsclient/client_async_error_integration_test.go` with the integration build tag. Start real NATS through the
   repository harness, connect a client with a capture logger, and install a real connection error callback that gates
   before delegating to the production `handleError`. Create a real raw async subscription with a one-message pending
   limit. Publish one message and wait until its handler is blocked; then publish a fixed additional count and flush.
   Wait until the real subscription's `Dropped()` equals the deterministically expected total, record that observed
   value, and only then release the error callback into `handleError`. Assert `ErrSlowConsumer`, exact subject, queue
   semantics, and `logged dropped == observed sub.Dropped() > 1`. This rejects a hard-coded presence value. Channels
   provide ordering; bounded polling/context provides a failure deadline; there are no arbitrary sleeps.
3. Materialize proposal/spec/tasks under `openspec/changes/attribute-nats-subscription-errors/`.
4. Keep [#954](https://github.com/C360Studio/semstreams/issues/954) as the E2E coverage-gap artifact. No existing
   product tier naturally and falsifiably forces a core-subscription pending-limit overflow; the real-NATS integration
   is the direct production-carrier proof.

No production file beyond `natsclient/client.go` is required. No ADR or schema generation is expected, but schema
generation remains a final no-drift gate.

## Required verification

```text
go test ./natsclient
go test -race ./natsclient
go test -race -tags=integration ./natsclient
task lint
go test -race ./...
task schema:generate && git diff --exit-code -- schemas specs
go test ./test/contract/...
openspec validate attribute-nats-subscription-errors --strict
```

Because `natsclient` is a shared substrate, final review also requires the repository-wide race suite and the project
contract/schema gates even though the production diff is small.
