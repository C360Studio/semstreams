...
**Status (revision 5):** inventory-only deliverable per `.agents/contracts/semstreams-architect.md` §"Required
...
in this revision (`(r3)`) and every item's disposition is in the design's checkpoint block. Round 2 (narrow re-review at `84cb6e3a`) returned
APPROVE WITH CHANGES; its text corrections were applied in revision 4. Owner ruling 2026-08-26 (comment on #1095): O-1 through O-11, O-13, O-14 accepted as recommended; **O-12 overridden to option (a) — foreign-authority imports are read-only mirrors**; the permanent hierarchy-inference skip for all foreign-authority entities explicitly accepted, on every lane. Revision 5 applies the ruling;
no inventory row changes except the R3 note.
...
| T6 (r3, re-anchored 2026-08-27) | `configs/graph-backend.json` and `configs/structural.json` compose graph-ingest (`grep -c '"graph-ingest"'` → 2 each), as do 10 more of the 14 shipped configs; only `gemini-example.json` and `prompts.json` do not | — | a reference import lane needs a config that composes graph-ingest; it goes in `configs/graph-backend.json`. **The original row measured `configs/cloud-federation.json`/`edge-federation.json` at 0/0 as the counter-example; PR #1130 (#1129) deleted both, so that evidence no longer exists and the point is restated on the surviving set.** |
