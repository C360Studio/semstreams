...
| 7 | file_input | input | `file_source` FilePort from Path/Format `input/file/file.go:200-205`; output from `config.ports[0]` `:206-212`; replace `:640-641` | CONFIG-DERIVED (replace + derived) | ERR `NATS client is required` (`:655`) |
...
| Factory default ports | `DefaultConfig()` in 31 of 33 factory packages (missing: `gateway/http`, `processor/rule`; §2.3 column 4) | declaration (code, not exported on `Registration`) |
...
| OpenAPI rows (generated) | `specs/openapi.v3.yaml:113-305` — ten operations; `/flowgraph` `:99` and `/gaps` `:332` belong to ComponentManager (`/flows`, `/flows/{id}`, `/validate`, `/publish-component-configs`, three `/observations/*`), schemas `Flow` `:1295`, `FlowCreateRequest` `:1380`, `FlowListResponse` `:1449`, `FlowUpdateRequest` `:1541`, Runtime*Response, `publishComponentConfigsResponse`, tags `:2202-2206` | — | regenerated by `task schema:generate` (`taskfiles/schema.yml:5-9`) | yes |
...
or `/components/validate`. e2e: 9 of 27 `*.spec.ts` reference `/flowbuilder` (`grep -rln flowbuilder e2e/`);
...
`flows` occurrences; `npm run generate-types:check` (`package.json:30`) diffs them. **Independent pass (2026-08-26) — call sites the prose
above does not name, all of which break under C:** `src/hooks.server.ts:26` (proxy path gate on `/flowbuilder`),
`src/lib/server/ai/toolExecutors.ts:355` (an AI tool executor GETs a flow),
`src/lib/components/runtime/OpsReadinessMatrix.svelte:32,56`. Totals by grep: 12 `src/` files / 16 sites non-test; the
migration list must be enumerated by grep, not from this prose.
...
**Compile against `main` `5cc0c7fb` (independent pass, scratch `go.work` replace, read-only): FAILS with 4 errors —
`main.go:300` unknown field `FlowEngineManager`; `:595` `NewEngine` 6 args vs 4; and two non-flow breaks, `:551`
`InitializeKVStore` now takes `ctx` and `:1079` `StopAll` takes `ctx`.** semteams is therefore already broken against
beta.162-to-be independent of #1089; C's marginal cost to it is the `FlowEngineManager` field, the three imports, and
the flow-template seeding, on top of a migration it already owes.
...
## 10a. Removal sweep items the independent pass added

- `persona/manager.go:29,46,68` reference `flowstore.Manager` in comments only — sweep on removal so no comment points
  at a deleted package.
- The objectstore constant instance name (`storage/objectstore/component.go:172`, §12.2) is a latent bug: if P1
  declarers are `f(config, instanceName)`, the admission parity check will either expose it or codify it — answer it
  before P1 lands (not before the ruling).
...
2. `resolveObjectStorePorts` uses the instance name for the `store-provide` port while the factory passes the
   literal `"objectstore"` (`storage/objectstore/component.go:172`); whether the admitted declaration ever carries
