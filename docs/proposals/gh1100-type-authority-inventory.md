...
| semdragon (beta.135) | `questdag.unit.v1` (`questdag/unit.go:72,673` on the **pre-ADR-091** subject `graphingest.SubjectEntityCreateWithTriples`); dynamic `semdragons.<event>.v1` written **directly** to `ENTITY_STATES` (`graphclient.go:24,79-95,108`) | **no** (registers `semsource.*` only, `semsource/payload.go:29-50`) | already off the current mutation surface; recorded in `docs/operations/migration-beta162-to-beta163.md` (not affected) |
...
7. Revision 5 (owner ruling 2026-08-26): sister obligations moved out of "notice" wording into the SemStreams-owned
   `docs/operations/migration-beta162-to-beta163.md` (O-11/O-12 override: sisters stay read-only); the seam finding N-5 is now
   ruled work (O-17).
