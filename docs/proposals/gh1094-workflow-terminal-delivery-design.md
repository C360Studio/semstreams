...
Phase: `inventory-checkpoint` → `pre-owner-design` (Part I is the inventory-only deliverable and stops for
`INVENTORY PASS`; Part II is drafted at the caller's request and is CONTINGENT on that pass — nothing in Part II
is approved, ruled, or accepted)

Body SHA-256: `41ad1f3c235698373ff95cabc6da69bf125123cf9078d9922489e2078fafbd69` (computed over every line below the `## Complete handoff body` marker)
...
| Persisted loop | `terminal_settlement.go:82-108` `loadPersistedLoop` reads `AGENT_LOOPS/<loopID>` directly via `natsClient.GetKeyValueBucket` (constant `agentLoopsBucket` at `http_activity.go:19-20`; no declared KV read port in `config.go`); `ErrKeyNotFound/ErrKeyDeleted` → transient (`:95-97`); malformed JSON / ID mismatch → permanent (`:101-106`) |
...
| Dedup contract | `natsclient/client.go:955-963`: dedup holds only within the USER stream `Duplicates` window (NATS default 2m when unset); the spec forbids claiming exactly-once (`openspec/specs/agentic-terminal-events/spec.md` "Delivery declaration SHALL remain bounded and honest") |
...
| `RunID` per `run_scope` (`new` mints a run rooted at the FIRING loop; `inherit`/`""` copies the firing entity's `agent.loop.run` triple; `none` suppresses) | `actions.go:1528-1615`; mint after validation `:1681-1716` via `agentrun.Mint`; `agent.loop.run` + `agent.run.entity-id` stamped on the FIRING entity `:1710-1714` |
...
| Framework-owned action names that exist today | exactly one: `needs_clarification`, synthesized on terminal-tool-less completion (`processor/agentic-loop/graph_writer.go:160-230`, `:181`; trigger `handlers.go:2011-2040`) |
| `respond_direct` / `ask_user` / `research` / `autoresearch` in framework code | `respond_direct`: 0 hits in `.go`; only `docs/concepts/25-phased-agentic-chains.md:59` as an example category. `ask_user`: comments only (`agentic-tools/config.go:26`, `decide.go:116,537`; `agentic/user_types.go` reply doc). `research`/`autoresearch`: only the `research` message domain and role strings |
...
| "published exactly once" (issue AC) | spec `agentic-terminal-events` forbids claiming exactly-once; dedup = `Nats-Msg-Id` within USER `Duplicates` window (`client.go:955-963`) | Wording does not survive; deliverable is one stable response identity per terminal |
| "`respond_direct`/`ask_user` decision" is a framework fact | §D: 0 code hits | Does not survive; it must become one (owner ruling) or be declared |
...
- `grep -rn 'respond_direct' --include='*.go' .` → 0. `grep -rn '"ask_user"' --include='*.go' . | grep -v _test | grep -v '//'` → 0 (comments only).
...
- `grep -n "KVReadPort\|KVWatchPort\|AGENT_LOOPS" processor/agentic-dispatch/config.go` → 0 (dispatch reads the bucket without a declared port).
...
**— end of inventory checkpoint; stop for `INVENTORY PASS` before reading Part II as anything but a draft —**
...
- Root-handoff suppression: unsolved — the root's own `AGENT_LOOPS` record and completion event never carry a
  `RunID` (mint stamps graph triples on the firing entity only, `actions.go:1710-1714`; `handlers.go:1991`), and the
...
- **R4 Origin resolution.** Only for a user-facing decision whose own route is empty. Walk from the persisted
  ...
  Dispositions: transient KV error → delayed NAK (`routing_read_transient`, unchanged); missing ancestor key →
  settle route-less with reason `origin_unresolvable` and a Warn naming the break loop id (redelivery cannot help:
  an ancestor's record predates its child, so absence means eviction); malformed ancestor JSON / id mismatch →
  permanent (`routing_malformed`, unchanged); partial route on an ancestor → permanent (unchanged).
- **R5 Identity.** Unchanged: `ResponseID` = `Nats-Msg-Id` = `terminal-user-response:<source id>`; timestamp =
  ...
  response identity per terminal, deduplicated within the USER `Duplicates` window" — never "exactly once".
...
| `processor/agentic-loop/handlers.go:2164-2170,1959-1993` | thread `toolResult` (name via `GetToolName`, metadata) into `handleCompleteResponse`; set `completion.Decision` when the tool is `agentic.DecideToolName`; replace literals at `:1921,:1935` |
...
| P4 | The loop knows its terminal tool's name at completion | `handlers.go:2164` (StopLoop) with `GetToolName(toolResult.CallID)` `:2241,:2302`; `TrackToolName` at dispatch `:1546` |
...
| P7 | Dispatch may read AGENT_LOOPS without a declared port today | `terminal_settlement.go:89`, `http_activity.go:200` (existing practice; a declared read port is a separate hygiene item) |
| P8 | Dedup is window-bounded, not exactly-once | `client.go:955-963`; spec "Delivery declaration SHALL remain bounded and honest" |
| P9 | AGENT_LOOPS keys expire 24h after last write | `component.go:761-766` |
| P10 | Run membership of the ROOT is not observable at the root's settlement | the decide triples land at `decide.go:407` before the tool result even returns to the loop, so the rule may fire and mint (`actions.go:1685`) before or after the completion is published (`handlers.go:2042-2054`) — the order is unconstrained; and the mint writes only graph triples on the firing entity (`actions.go:1710-1714`), so the root's own `AGENT_LOOPS` record and its completion event never carry a `RunID` (`handlers.go:1991` copies `entity.RunID`, which is set only from `TaskMessage.RunID` at `:476`) |
...
1. **Home of the reply vocabulary.** (a) Framework-reserved `respond_direct` + `ask_user` as a cross-repo contract
   (recommended; ADR draft staged as `docs/adr/100-…`), or (b) a dispatch config list defaulting to those two
...
2. **Should `needs_clarification`** (the framework-synthesized action, `graph_writer.go:181`) be user-facing
   (`prompt`)? Recommended: no in this change — it is a routing marker for rule packs; flipping it publishes a
   prompt for every text-only coordinator completion.
...
4. **Is any in-run `ask_user` user-facing**, including from a non-terminal loop? Recommended: yes — that is the
   pause/resume shape gh#256 built (`http.go:42-46`).
...
10. **Scope hygiene, separate:** dispatch reads AGENT_LOOPS without a declared KV read port (`config.go`); file or
    fold?
...
2. Issue AC "published **exactly once**" → the current spec forbids the claim; the deliverable is one stable
   response identity per terminal, deduplicated within the USER `Duplicates` window (`client.go:955-963`).
...
10. Unstated by issue and triage: AGENT_LOOPS key TTL 24h (`component.go:766`) bounds any origin resolution that
