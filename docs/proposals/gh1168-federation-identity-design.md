...
One more writer to name: `semstreams_config` has a fixed global name and this package is not its only creator —
`processor/rule`'s own ConfigManager creates it for `rules.*` keys (`processor/rule/kv_config_integration.go:574`),
and `cmd/semstreams/main.go:733-736` documents two ConfigManager instances coexisting against it by design. The
third branch's refusal therefore names two possible causes, not one; the remedy is identical for both.
...
| 3 | mint-vs-budget: `validateAuthorityPair` bounds the *unsuffixed* pair; the suffix adds 7. A pair that fits unsuffixed but not suffixed passes load, is durably `Create`d, and then fails forever — d7 forbids rewriting (P9) | `config/config.go` `validateDeclaredAuthorityPair` / `validateAuthorityPair` | **bound at load the value that will actually be minted**: `len(org) + len(id) + 7 ≤ MaxAuthorityPairBytes()`, applied at the DECLARATION boundary (`Loader.Load`/`LoadFromBytes`, ungated like `rejectRemovedPlatformFields`). An effective pair — minted, adopted, or running — already carries the suffix and is bounded at the full 170. The reserve is a fact about a declaration, not about a pair: applying it to both kinds reserves the same seven bytes twice and refuses at Start a declaration that had already passed load (the first implementation did exactly that for every pair in (156, 163]; caught in review, `TestMaximumDeclarablePairMintsAndStarts` pins it). No pre-`Create` probe and no rollback: a rollback would `Delete` an identity record, which d7 forbids. Cost stated: 7 bytes of declarable headroom (163 rather than 170; the longest pair measured in this family of repos is 33) |
...
| Authority-pair budget | `len(org)+len(id) ≤ 170` everywhere | **declared** `len(org)+len(id)+7 ≤ 170` (163 bytes, at the load boundary only); **effective** unchanged at 170; `TestConfigRejectsOversizedAuthorityPair` 171 → 164 | collision 3 |
