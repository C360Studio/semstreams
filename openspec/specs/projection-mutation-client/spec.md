...
`projection-mutation-client` governs the **public, contract-bound write path** into the graph — the
framework's answer to "how does a non-rule consumer mutate projections without assembling raw NATS
requests, retry policy, provenance, owner tokens, and read-back itself."

It owns a deliberately narrow capability set: contract-validated atomic creation, schema-derived
replacement of owned predicate groups, append-only evidence, and authoritative read-back. Ownership
is established before any owning write binds, and the client returns a typed mutation error plus a
**commit state** — committed, not-committed, or unknown — rather than collapsing an ambiguous
transport outcome into a bare error. That commit-state distinction is the reason this capability
exists: a caller that cannot tell "did not commit" from "cannot tell" will either duplicate work or
strand it.

**What it does NOT cover.** The mutation verbs themselves, their merge semantics, and add-lane
deduplication belong to `graph-ingest`. Rule-initiated mutations bind through
`rule-projection-mutations`. Ownership registration and lease semantics are the ownership
capability's. This is the client-side contract: what a caller may ask for, and what it may conclude
from the answer.
## Requirements
...
The client MUST NOT claim exactly-once behavior or prove absence after a timeout. Generic outer retry loops MUST
be prohibited.

The late-commit duplicate this requirement was originally written against is CLOSED. gh#697 landed
(PR #747, 2026-07-30): the graph-ingest add lane now deduplicates unconditionally by the same six-field
tuple this client keys on, from one shared implementation, so an identical retry after a late commit
stores exactly one tuple. `Retry.MaxRetries=0` is therefore no longer required to avoid a duplicate, and
the client MUST count a server-reported `Deduplicated` toward the request being fully accounted for
...
- **AND** if the original attempt commits late, the server suppresses the retry's identical tuple, so exactly
  one is stored
