...
After `config.Manager.Start` completes selection and synchronization, service composition SHALL use the resulting
`SafeConfig` and SHALL NOT use the stale original file-loaded config object.
...
schema interpretation, or codec callbacks. Constructors SHALL receive exact canonical raw config and SHALL solely
validate/default enabled service inner config. Disabled-only resolution SHALL invoke no constructor or inner validator.
...
Outer `ServiceConfig.Enabled` SHALL be the sole activation input for optional services. A configured
