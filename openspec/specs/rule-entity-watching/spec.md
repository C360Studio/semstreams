# rule-entity-watching Specification

## Purpose

`rule-entity-watching` governs how the rule processor **watches entity state and decides what to
evaluate**. Evaluation is gated by authoritative graph-state validation, so a rule never fires on
state the graph would reject. Dynamic watcher authority is atomic and generation-scoped — a watcher
recreated under churn cannot have a stale generation resurrect work the new one already superseded.
Coalesced entity work preserves provenance and collapses to exactly one current evaluation, and a
single bounded per-entity fence orders revisions against deletion.

The unifying concern is **ordering under concurrent change**: entity updates, watcher recreation,
and deletion race each other, and each requirement here names one way that race produced a wrong
evaluation or a lost one.

**What it does NOT cover.** Condition evaluation semantics belong to `rule-engine`; what a rule may
write belongs to `rule-projection-mutations`; entity-ID validity belongs to `entity-id-contract`
(explicitly scoped out of this change). This capability is the watch-and-dispatch boundary.
## Requirements
