...
**SIMPLIFICATION**. Line numbers are at `f6a4dbeb` unless stated.

| Ruling | Landed as | Where |
|---|---|---|
| **Q1** birth order — Put → publish; a task redelivered at iteration 0 republishes R1 | Birth writes the record by `Create` and publishes after; the cold task fork rebuilds R1 through the ordinary birth path. **DEVIATION — see the row below.** | `component.go:2880` (`createLoopState`, `Create` at `:2896`), `component.go:1436` → `loop_classification.go:117` (`classifyRedeliveredTask`), republish arm `component.go:1573` |
| **Q1 DEVIATION** — "republish it unconditionally with the MsgId, no retained read" | The rebuilt R1 goes out through `publishResults`, which consults `adoptRetainedRequest` first, so an R1 the stream already retains is **adopted, not republished**. The birth is never blocked behind a read (Q1's intent); the one divergent state — R1 retained, dedup window expired — yields no duplicate instead of a second copy under the same name, which is what task 2.3 exists to prevent. **Owner-ratified 2026-09-22**, verbatim "1330 agree with recommendation": [issuecomment-5776942078](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5776942078). Q1 is amended to the adopt-not-republish reading. | `component.go:2686` (the `msg.MsgID != ""` arm of `publishResults`) → `loop_evidence.go:340` (`adoptRetainedRequest`); evidence `task_redelivery_integration_test.go` |
| **Q2** carrier form — non-terminal carrier becomes `Update(observedRevision)` | `persistLoopState` compare-and-swaps against the revision this process observed, for every caller; the ORDER flip is scoped to the task, model-response and tool-result lanes (coordinator scoping under OQ7/OQ8) | `component.go:2921` (`Update` at `:2967`), order at `component.go:2225` (`publishThenPersistResultState`) |
| **Q3** applied set — `PendingToolResults` keys ARE the applied execution IDs; no separate field | No field added; the map's keys are the applied identities and the rebuild reads them as such | `agentic/state.go:57`, read at `state.go:497` (`restoreToolBatch`) |
...
| **Q5** duplicates — RequestID as `Nats-Msg-Id`, the window is a bonus | Every publish carrying a MsgID goes through `PublishToStreamWithMsgID`; identity adoption, not the window, is the guarantee | `component.go:2700` |
| **Q6** verdict ACK for a verdict with no waiter | **L4b (#1362).** Out of L4a scope by OQ7 | `tasks.md` § "Moved to L4b (#1362)", bullet 3.6 |
| **Q7** terminal + unproven — effect-free ACK with a metric and an audit line | Component-entry classification acks without effect and counts `terminal_unproven` / `older_request` on the existing `tool_results_dropped_total`; each drop carries a `WarnContext` naming both request names | `loop_classification.go:174` (`classifyRedeliveredToolResult`), counters `:187` and `:203`, warm call site `component.go:2492` |
| **Q7 recorded deviation** — the terminal arm does not check `PendingToolResults` membership first | Membership would only tell "already applied" from "never applied" for a loop that can apply neither, and Q7 rules both to the same effect-free Ack; the branch's two sides would do the same thing. Widened in the delta's terminal scenario so the GIVEN covers both | `specs/agentic-loop/spec.md:78-86`, `tasks.md` 3.5 |
...
| **OQ2** no approval-deadline hydration; the sentence becomes a spec scenario and a migration line | **CONFORMS, with a recorded narrowing owed on the requirement's prose.** No hydration is built: `SnapshotExpiredApprovals` reads `m.loops`, and no startup path reads the bucket. The scenario, the migration section and the measured-delta test all landed. The requirement's free-text sentence says "a replaced process SHALL re-arm no approval deadline" without qualification, and that is measurably too broad — a replacement that rebuilds a loop for another reason (task 1.2) seats its `PendingApproval` with it. The scenario, which is the testable form, is scoped to startup and is exactly true. The narrowing is the owner's to ratify because the sentence is verbatim ruling text; it is NOT applied here | scenario `specs/agentic-loop/spec.md:58-63`; test `approval_deadline_hydration_integration_test.go:34` (`TestAReplacementReArmsNoApprovalDeadline`); migration `docs/operations/migration-beta162-to-beta163.md:1758`; sweeper `state.go:630`, doc `approval_sweeper.go:40-46`; the over-broad sentence `specs/agentic-loop/spec.md:36-38` |
| **OQ3** a CAS loss releases the loop's process state and Retries; birth by `Create` | Birth is `Create`; a lost CAS releases this loop's in-process state and returns the sentinel, which the lane reads as Retry | `component.go:2896` (`Create`), `component.go:2967-2973` (release + `ErrKVRevisionMismatch`), lane reads at `component.go:2190`, `:2236` |
| **OQ4** document `tasks_submitted_total` as at-least-once under redelivery; no arm change | No arm changed, and the `specs/agentic-dispatch` delta IS in the tree (landed `0e327a51`). What is still open is the adopter half: the migration-note line and the dispatch-side test carrying the `// spec:` citation, both **task 5.4** | delta `specs/agentic-dispatch/spec.md` (`design.md:209` carries the counter semantics); counter `processor/agentic-dispatch/metrics.go:112`; open half `tasks.md` 5.4 |
| **OQ5** classify the redelivered tool result at component entry; `TransitionTo` untouched | The terminal and ordering checks sit before `HandleToolResult`; `LoopEntity.TransitionTo`'s same-state `nil` is left alone | `component.go:2492`, `agentic/state.go` `TransitionTo` unchanged |
