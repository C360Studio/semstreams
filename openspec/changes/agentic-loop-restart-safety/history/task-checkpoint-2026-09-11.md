# Historical task checkpoint: agentic-loop restart-safe settlement

Frozen before the owner-approved 2026-09-11 documentation reconciliation. The original task file had SHA-256
`ef4f75127810325914343e7facd9b891bb26d7020f0f96c9f5ef789dd4ca9aca` at HEAD `160ad091` plus unpublished changes.
Apart from this heading and notice, its text is preserved verbatim. Paths below resolve from the original change
directory unless absolute. This is evidence and superseded execution history, not current instructions, a fresh
test result, or permission to implement an old proposal. Do not append new progress here. Current scope and task
truth live in [proposal](../proposal.md), [design](../design.md), [tasks](../tasks.md), and the capability deltas.
...
The next test-only batch from checkpoint `160ad091` produced two GREEN controls and one measured RED.
Native original-status replay after MODIFY passed in 30.54s (package 32.844s); actual timeout replay passed in
30.47s (package 32.368s), preserving the original 8s deadline despite replacement configuration of 1m. Their
complete log SHA-256 values are `d06ba237cdb8db8e1b1673dcc0ae2670f504052b11a39a8c5825b6010e228e07` and
`beee3604ac31fe313c75a350a5f5a67cffecf94ffff28f80689e6c1eeec29ea6`.
The matching-pending native retry passed its pre-ACK witness: original source sequence 6, delivery 2, unchanged
KV revision 3 and bytes, prompt sequence 7 to 8 with identical payload and original 1m deadline despite replacement
configuration of 2m, then source ACK 1 / NAK 0 / Term 0. It failed the subsequent dispatch notification drain before
any HTTP decision: the approval-pending consumer's ACK floor remained 7 instead of reaching 8. The complete RED
log SHA-256 is `d9257c055dbc3b1c63f2dbd5706fad60da626acde82a568ad72808a08958934e`
(test 35.47s, package 36.286s). Logs are in `/private/tmp/gh1146-native-control-batch.jdPmZ0`.
`handleAgentApprovalPending` retries when `LoopTracker.SetPendingApproval` buffers an unknown loop; the replacement
tracker cannot recover its already-ACKed `agent.created` input. This is the existing dispatch projection/consumer
retirement boundary in tasks 6.7-6.10, not missing loop PubAck, a lost deadline, or evidence for a new Store mechanism.
Independent review approves the test fidelity at source SHA-256
`b2d46fed30d8d8b7f53f5b43079ac54d58d798c9304d8ac8d741a3857104107d`, not a GREEN batch.
Preserve the drain assertion and the RED regression. No production correction, full pre-push run, push, task
completion, parent advance, or task 6.6 ruling is claimed by this batch; sequencing the dispatch work returns to
the owner. The earlier stale-closure/restoration, retained-absence, and broader evidence obligations remain open.

On 2026-09-11 the owner answered **"yes bring it forward please"** to bringing the planned durable-state dispatch
cleanup forward instead of patching the tracker scheduled for deletion. Execute the existing coherent dispatch
slice in 6.7-6.13 next, preserving the pending-retry RED as the acceptance case and removing its obsolete dispatch
consumers rather than fabricating their settlement. The approved design and target requirements do not change.
The task 6.6 Store ruling, remaining approval evidence, frozen parent, and merge/closure gates remain separate.
No checkbox is completed by this sequencing change; source/evidence review and the applicable gates still apply.

The dispatch implementation checkpoint found one direct validation prerequisite: `LoopEntity.Validate` still
accepted `paused`, while the approved current-record requirement explicitly refused it. Correct that acceptance in
the existing shared validator, with direct validation and dispatch-read/projection tests, rather than inventing a
dispatcher-only state allowlist. This brings forward only that validation clause of 7.2-7.3; complete constant,
transition, schema, example, and documentation retirement remains unchecked under 7.2-7.4. It does not revive pause
semantics or establish compatibility support. No runtime or proof completion is claimed by this dependency note.
The direct validator test was RED (0.485s), and the workspace guard refused the attempted shared production change
as outside the selected dispatch slice. No production hunk was applied. A separate owner question asked whether
to bring that shared validation correction forward; the RED and full push-gate hold were retained pending the answer.
Unrelated dispatcher cleanup continued. The exact rejected patch is retained as
`rejected-paused.patch.txt` in `/private/tmp/gh1146-dispatch-retirement.843CEt`, SHA-256
`d16151fbf56d720ad8d0295af8e267f442681ec2077be53b93a562611081d999`.

The unpublished cleanup was frozen at the safe pause described in
`/private/tmp/gh1146-dispatch-retirement.843CEt/pause-handoff.md`, SHA-256
`2ea793696409af7e1bd7b25052f318d72ed67bcd13e43d6ef2f14e700d8378b5`.
Graphview race is GREEN (1.613s) and independently approved at its six-file manifest
`b8021662b717652d71afb24eaae13e12701a059260e40d6f217d6585f7d7909a`. That dispatch race remained RED only at
`TestStatusReportsTheRecordedStateNotAFabricatedRunning/invalid_state_paused` (1.263s); E2E fixture unit controls
pass (1.533s), not application E2E. The changed native fixtures compile but have not run. Research processor
tests also failed embedded-NATS startup; one unchanged representative passed with explicit local networking,
which does not establish full-group GREEN. Exact logs, source copies, moved-test provenance, and proof gaps are
in the handoff. Lookup/HTTP controls, native mixed-bucket/shared-view, pending retry and retention proofs, generated
schema review, combined review, full push/E2E gates, and the shared-validator approval remained open at that pause. No source
commit, push, checkbox completion, parent advance, or broader mechanism ruling is claimed. All agent sessions joined.

On 2026-09-11 the owner answered **"yes"** to the explicit question asking to reject retired `paused` in the shared
validator for all `LoopEntity` callers. The exact scope is recorded in
[issue comment 5637482126](https://github.com/C360Studio/semstreams/issues/1146#issuecomment-5637482126).
This lifts the shared-validation scope hold only. Resume the original shared-validator correction and its direct
and dispatch regressions, then the existing dispatch proof sequence. Full paused-vocabulary retirement, task 6.6,
parent cutover, and all unrun review/test gates remain separate. Approval alone does not complete a task or establish GREEN.

The approved correction is now applied and independently reviewed: `isValidLoopState` no longer accepts `paused`;
all other states and transition APIs are unchanged. Direct validation, shared-view poison, exact-reader refusal,
and the existing recorded-status regression pass. The exact-reader control injects the loader result; it is not
a native KV proof. `go test -race ./agentic ./processor/agentic-dispatch -count=1 -timeout=60s` passes (5.104s and
2.152s respectively). Three-file review manifest SHA-256
`070e035fe11141858e90091bd3d13cea9564580ce524b30f831ed5297647099c`; retained log SHA-256
`a5f6f5c6d3a360872826b29f5494b446e825ec8e2bfaa571d65677aee9b43dd9`, both in
`/private/tmp/gh1146-validator-resume.9SLJyk`. This resolves only the validation prerequisite, not mixed task
6.10 or full 7.2-7.4, the remaining native proofs, generated-output review, full gates, or task 6.6.
...
- [ ] 6.8 Add a real mixed-bucket proof covering valid current `LoopEntity`, typed terminal `COMPLETE_`, registered
  ...
  contains no aggregate-to-directional token mapping. Preserve the accepted edge-gateway value-mapping exception:
  optional `context_request_id` is empty because it has no durable authority. These exceptions permit no other
  projection expansion or mapping change.
  Cite exactly
  `// spec: agentic-dispatch / The shared loop view classifies the mixed bucket`.
...
- [ ] 6.10 Replace `CommandContext.LoopTracker` with classified `LookupLoopOwner`. Preserve `LoopInfo` only as the
  ...
  except for `execution_id` on existing nested `PendingApprovalInfo`. Keep the accepted edge-gateway exception that
  optional `context_request_id` is empty; preserve all other unrelated fields and mappings.
