...
- [ ] R2 Complete approval replacement/replay evidence and its remaining corrections (old 6.1, 6.1a, 6.2, 6.3, 6.5,
  ...
  mutation; same-gate contention; and missing/unreadable/malformed evidence classifications. The latest focused
  matching-pending replay is green after tracker removal, not a full batch gate. Actual newer-closure versus stale
  restoration ordering and the confirmed-retained-absence `continuation_unavailable` consequence remain open.
  Reconcile remaining native later-history/ordinal coverage honestly; do not relabel seeded/unit evidence native.
  Preserve diagnostic-omission negatives and the explicit approval-after-OS-restart E2E stage.

- [ ] R3 HOLD: resolve the existing approval mechanism ruling from R2 evidence (old 6.6). Only an explicit owner
  revocation of comment `5463183450` after the evidence gate permits removing ApprovalContinuationV1, Store config,
  digest, cleanup, and deliberate AGENT-eviction claims. Otherwise retain the already-approved Store plan.
  Introduce no third mechanism. This documentation reconciliation does not supply the ruling.

- [ ] R4 Close loop task/response and remaining no-premature-ACK proof (old C.4, 4.1–4.3). Account for birth/lineage,
  ...
  Implemented read-through/heartbeat paths still need complete claim-set evidence, not another owner/runtime.

- [ ] R5 Close tool-result and completed-effect recovery proof (old 5.1–5.3). Preserve globally correlated execution
  ...
  replacement boundaries. Update the three named current tool requirements without losing unaffected scenarios;
  add no claimed/in-progress ledger. Prior ordinary/direct-terminal proofs do not complete this whole group.

- [ ] R6 Finish cancel/approval/verdict fast lanes and retired vocabulary (old 7.2–7.8). Remove the remaining paused
  constant, transitions, schema/examples/docs and reject it without a compatibility path. The shared validator
  correction is already narrowly approved, not full vocabulary completion. Prove cancel's required current state,
  COMPLETE_ and terminal PubAck; unknown-signal refusal; missing/full waiter, conflict, panic, replacement and
  pre-marker failure. Preserve one release point, lane-specific applied proof, approval exceptions, and nonblocking
  audit/graph evidence. ResponseAction/intent hints are not additional durable signal verbs.
