...
| Accepted ruling | Implementation evidence |
|---|---|
| Keep the retired NATS factory absent | No Go source references `NewNATSLessonCurator`; `cmd/e2e-semstreams/main.go` constructs one local mutation client and injects it through `NewLessonCurator`. |
| Export one purpose-scoped snapshot | `processor/agentic-tools/lesson_promotion.go` exposes only `LessonProjectionContract` for this contract surface. |
| Share one canonical declaration | `internal/builtinprojection/contracts.go` derives both `Contracts()` and the public adapter from `LessonContract()`. |
| Return independent nested slices | `processor/agentic-tools/lesson_promotion_test.go` mutates the contract, birth predicates, group list, group names, and each group predicate slice before comparing a later snapshot with the canonical value. |
| Preserve narrow curator injection | `NewLessonCurator` still accepts only `PredicateReconciler` and `AuthoritativeReader`; the real-NATS integration test injects the locally constructed client through those capabilities. |
