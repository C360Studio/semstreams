...
| # | Premise | Measurement |
|---|---|---|
| P1 | The `order` argument of `persistHandlerResult` is dead — for every result that publishes anything. | Terminal results return at C:2278-2282 before `order` is read at C:2285; the three `writeThenPublish` callers sit behind `result.State.IsTerminal()` (ARH:210→221, AS:113→123, C:2659→2664). For a non-terminal result `order == publishThenWrite && !gated` (C:2285) ≡ `!gated`. **Amended in implementation review (PR #1381, 2026-09-25):** `IsTerminal()` includes `cancelled` (`agentic/state.go:44-45`) and the carrier's terminal branch does not (complete/failed only, C:2246), so ONE non-terminal-at-the-carrier pair did reach the order read with `writeThenPublish`: a cancel that lands between the approval's resolve (ARH:59) and its `GetLoop` (ARH:85) gives `State: cancelled` (ARH:90); once the cancel lane has released the loop, `dispatchApprovedCall` fails in `AddPendingTool` (`handlers.go:2036`, "loop … not found"), wrapped at ARH:139, and ARH:210 sends it to the carrier. The zero-change claim survives because that producer publishes nothing — every `dispatchToolCall` failure returns before its append, and ARH:90 starts the result with no publications — so write-then-publish and publish-then-write run the same side effects in the same order on it. Only the cause text of the write failure changes: "handler result state has unknown durability after the loop was already mutated" → "… after its results were published". Disposition: Quarantine either way (the write fails on the released loop). The row is pinned in `transition_result_test.go` ("D3 cancelled"). |
...
- **`persistHandlerResult(ctx, result)`** loses `order` (C:2245). Its body keeps the terminal branch first (C:2278), then
  ...
  and terminal orders are preserved exactly — P1 shows the one non-terminal pair that passed `writeThenPublish` (a
  cancelled result racing an approval) publishes nothing, so both orders are the same on it; only its cause text moves.
