...
The `natsclient.Client` JetStream publish surface: how a producer writes messages
onto a stream, and the invariants every publish path preserves (distributed-trace
propagation, server-side dedup, circuit-breaker health accounting).

> Seeded lazily by the gh#470 async-publish change. Requirements distilled from
> `natsclient/client.go` and verified against code. Scope is the **publish** path
> only; consume/KV/stream-management are separate capabilities, seeded when a
> change first touches them.
