...
- [x] 2.3 Drop the argument at the nine test call sites: `processor/agentic-loop/loop_carrier_test.go:76` — `err := c.persistHandlerResult(t.Context(), nonTerminalResultWithAPublication(loopID), publishThenWrite)`,
      ...
      **Done, and the count was 26, not 9:** the list above named 9 of the 32 test hits the inventory counted (inventory § carrierOrder, "32 in tests across 14 test files"). The argument was dropped at all 26 `persistHandlerResult` test call sites in 14 files (7 of those files are `//go:build integration`, compiled by `go vet -tags=integration ./...`, exit 0). Non-terminal callers that had passed `writeThenPublish` and now take publish-then-write: `publish_phase_fatal_test.go:48`/`:57` (unit, still green) and `partial_publish_settlement_integration_test.go:53`/`:69`. Implementation review H1 (PR #1381): that test's third segment asserted the write-first cause and had passed for the wrong reason (no record seeded, so `persistLoopState` refused on "no observed record revision" before any publish); it now drives a result whose publications all land, with the revision seeded, and asserts "after its results were published" and "kv unavailable". `go test -tags=integration -race -count=1 -v -run TestIntegrationPartialPublishQuarantinesRatherThanRetrying ./processor/agentic-loop/` → `--- PASS` (1.35s), `ok`; with the seed removed (`cp` backup, md5 restored) it fails at `:116` on "does not contain \"kv unavailable\"".
