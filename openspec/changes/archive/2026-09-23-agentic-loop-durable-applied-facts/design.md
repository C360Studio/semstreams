...
> UUID-minted at `68c14c8e` (`state.go:1371-1365`) and `af829616` (`state.go:1138-1132`); deterministic IDs shipped with
...
15. A LIVE (non-deferred) attach can mint concurrently with the tool lane — **residual, observed not designed**
    (round 3 docket, beside finding 2). In the interval between `RemovePendingTool` (`handlers.go:2692`) and
    `TrackRequest` (`handlers.go:3087`) both `outstandingRequests` and `pendingTools` are empty for the loop, so a
...
five findings of the owner's SECOND Codex round
([issuecomment-5790425046](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5790425046)), and the
three code findings of the in-house sweep run before round 3 (§ 8.7 of `tasks.md`) — three
fixed as deviations from a ruling or the design, and two answered by the owner's ruling of 2026-09-23
([issuecomment-5791390564](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5791390564), verbatim
"agree on both - continue") — and the two remaining findings of the owner's THIRD Codex round
([issuecomment-5792346704](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5792346704)), whose
findings 1 and 4 the in-house sweep had already fixed as B1 and M5, plus the response-replay residual that sweep
traced and the owner ruled on as Q12 (§ 8.8 of `tasks.md`), and the three findings of the owner's FOURTH
Codex round ([issuecomment-5794412825](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5794412825),
§ 8.9 of `tasks.md`).
Every row points at a line in this tree or names the change that carries it. One row is a **DEVIATION**; one is a
**SIMPLIFICATION**. Line numbers are at `3e9f7ba3`, the last commit of round 4 that touches a Go file,
re-derived across the `0bbee770` → `3e9f7ba3` diff rather than transcribed (checkpoint-4 review, MEDIUM-3); the
records commit that follows it changes markdown only. Every pin the round's fix commits moved was re-derived by reading its OLD line at the previous head and finding that exact text again, never by arithmetic. Every pin the round's own commits moved was re-derived from the symbol it names, not by arithmetic. Unlike `tasks.md`'s task descriptions, nothing here is baseline evidence: every row names where a ruling landed in THIS tree, so every pin moves with the tree.

| Ruling | Landed as | Where |
|---|---|---|
| **Q1** birth order — Put → publish; a task redelivered at iteration 0 republishes R1 | Birth writes the record by `Create` and publishes after; the cold task fork rebuilds R1 through the ordinary birth path. **DEVIATION — see the row below.** | `component.go:3057` (`createLoopState`, `Create` at `:3080`), `component.go:1485` → `loop_classification.go:142` (`classifyRedeliveredTask`), republish arm `component.go:1631` |
| **Q1 DEVIATION** — "republish it unconditionally with the MsgId, no retained read" | The rebuilt R1 goes out through `publishResults`, which consults `adoptRetainedRequest` first, so an R1 the stream already retains is **adopted, not republished**. The birth is never blocked behind a read (Q1's intent); the one divergent state — R1 retained, dedup window expired — yields no duplicate instead of a second copy under the same name, which is what task 2.3 exists to prevent. **Owner-ratified 2026-09-22**, verbatim "1330 agree with recommendation": [issuecomment-5776942078](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5776942078). Q1 is amended to the adopt-not-republish reading. **Superseded ON THE TASK LANE by Q11 (2026-09-23), the row below:** the task lane no longer reaches the publish path's adopt at all, because it no longer seats a loop over a retained R1. The deviation stands as written for every other minting lane. | `component.go:2863` (the `msg.MsgID != ""` arm of `publishResults`) → `loop_evidence.go:340` (`adoptRetainedRequest`); evidence `task_redelivery_integration_test.go` |
| **Q2** carrier form — non-terminal carrier becomes `Update(observedRevision)` | `persistLoopState` compare-and-swaps against the revision this process observed, for every caller; the ORDER flip is scoped to the task, model-response and tool-result lanes (coordinator scoping under OQ7/OQ8) | `component.go:3173` (`Update` at `:3225`), order at `component.go:2362` (`publishThenPersistResultState`) |
...
| **Q5** duplicates — RequestID as `Nats-Msg-Id`, the window is a bonus | Every publish carrying a MsgID goes through `PublishToStreamWithMsgID`; identity adoption, not the window, is the guarantee | `component.go:2877` |
| **Q6** verdict ACK for a verdict with no waiter | **L4b (#1362).** Out of L4a scope by OQ7 | `tasks.md` § "Moved to L4b (#1362)", bullet 3.6 |
| **Q7** terminal + unproven — effect-free ACK with a metric and an audit line | Component-entry classification acks without effect and counts `terminal_unproven` / `older_request` on the existing `tool_results_dropped_total`; each drop carries a `WarnContext` naming both request names | `loop_classification.go:283` (`classifyRedeliveredToolResult`), counters `:294` and `:310`, warm call site `component.go:2637` |
| **Q7 recorded deviation** — the terminal arm does not check `PendingToolResults` membership first | Membership would only tell "already applied" from "never applied" for a loop that can apply neither, and Q7 rules both to the same effect-free Ack; the branch's two sides would do the same thing. Widened in the delta's terminal scenario so the GIVEN covers both | `specs/agentic-loop/spec.md:112-130`, `tasks.md` 3.5 |
...
| **No deprecation** ([issuecomment-5774793150](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5774793150), coordinator-applied under the standing #759 ruling) — the two truncation-retry helpers are DELETED in this PR, not marked `Deprecated:` | **CONFORMS.** `LoopManager.IncrementTruncationRetry` and `ResetTruncationRetry` are gone from `state.go` together with the `truncationRetryAttempts` map; the budget is read back off the retry ordinal of `PublishedRequestID`. The change is a `fix(agentic-loop)!:` with a migration-note section, and `task api:compat:report` lists both removals under `processor/agentic-loop`, joining the three entries earlier layers left | `git grep -n 'IncrementTruncationRetry\|ResetTruncationRetry' -- '*.go'` is empty; migration note `docs/operations/migration-beta162-to-beta163.md:1731`; `handlers.go` `handleLengthTruncation` reads the ordinal |
| **Carrier-reorder scoping** ([issuecomment-5773929421](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5773929421), coordinator-applied under OQ7/OQ8) — L4a reorders publish → CAS `Update` only on the task, model-response and tool-result lanes; the writer still becomes `Update` for every caller | **CONFORMS.** One writer for everyone; the order flip is lane-scoped. The approval lane's call site and any result that CREATES an approval gate keep write → publish, and the approval-timeout sweeper keeps the publish-then-`Put` pair it already had (§ 5.6 and `AS:104`), because the reject-minted W4 those reorders open is closed only by the approval lane's cold branch (#1362, task 3.3). The gate arrives on the TOOL-RESULT lane, which asks for publish-first, so the carve-out is in the carrier rather than at a call site — round 2, finding 2 | writer `component.go:3173`; flip `component.go:2362` (`publishThenPersistResultState`), reached from `component.go:1903` and `:2694` only, and skipped for a gate by `component.go:2288`; approval lane `approval_response_handler.go` unchanged; `approval_sweeper.go` keeps its ORDER and gains the carrier's stamp between its publish and its write (in-house sweep before round 3, B1); round 4, finding 1 makes both the stamp and the write conditional on that publish having landed, which is the carrier's own shape for the same error; scoping `design.md` § 1, deferral `tasks.md` § "Moved to L4b (#1362)", bullets 2.1 and 2.7 |
...
| **Owner Codex round, finding 3** (2026-09-22) — a sibling lane can commit a request ID before that request is published | **FIXED.** The memory stamp of `PublishedRequestID` moved from the two ITERATION mint sites to the carrier: `stampPublishedRequest` runs under `loopRecordMu` after `publishResults` has PubAck'd (or adopted) the minted request and before `persistResultState` reads the entity. Stamped at the mint, the name was on the SHARED entity from the moment the request was built, so a deferred continuation's write on the task lane or a tool lane's CAS could commit a record naming R2 over a stream retaining only R1 — and a crash there leaves a loop no replacement can adopt and no operator can settle. **I1 is now true by construction** rather than by every mint site remembering the order: the field is only ever written by a step that runs after the request is retained. **Scope corrected by round 3, finding 2 (the row further down):** that holds for the NAME and for nothing else — `Iterations` and `PendingToolResults` still moved on the shared entity ahead of the publish, and a sibling render still committed them, which is why the deferred lane stopped rendering the live entity. `TrackRequest` stays at the mint (route, outstanding, the deferred turn's carrier are attach-order facts); birth keeps its mint-site call, because Q1 writes its record before the first publish. The stamp is at the carrier for EVERY lane, including `writeThenPublish` ones — the approval-rejection path reaches `publishIterationRequest` through `HandleToolResult` and would otherwise never name its minted request — and that order is unchanged for them | stamp `component.go:3145` (`stampPublishedRequest`), selector `component.go:3098` (`mintedRequestID`), call sites `component.go:2371` (publish-first) and `:2325` (write-first); mint sites `handlers.go` `publishIterationRequest` and `emitRetryRequest` no longer call it, birth `handlers.go:1168` does; doc `state.go:1123` (`SetPublishedRequest`), field doc `agentic/state.go:68`; design § 3 item 4; test `loop_record_writer_test.go` (`TestARecordNeverNamesARequestBeforeItsPubAck`), commit `99cef493` |
| **Owner Codex round, finding 4** (2026-09-22) — recovery loses an acknowledged deferred user turn | **DOCUMENTED (owner ruling Q2/Q3) plus the two-line honesty fix.** The turn's text lived in the predecessor's context manager and is not recoverable; `PendingContinuationRequestID` is empty precisely because no request carried it. `restoreLoopFromRequest` now clears `PendingContinuation` in that case and warns, so a rebuilt loop does not spend an iteration re-asking the model with a context that gained nothing — a turn already inside a retained request is untouched, because the replay carries it. The loss is the documented limitation in one sentence family at three homes, and **Q8 rides it**: `taskPrompts` is the one per-loop cache the rebuild does not restore, so `LoopCompletedEvent.Prompt`, `LoopFailedEvent.Prompt` and `recoverEmptyContext`'s fallback all read empty after a replacement. The durable-turn and durable-prompt fields are an exported-surface addition to a Tier 1 package and are their own issue, #1365. Anti-gaming condition met: the limitation is a delta scenario AND the regression asserts both halves of it | clear `state.go:416`; `doc.go:276` § Recovery across a process replacement; migration `docs/operations/migration-beta162-to-beta163.md:1779`; delta scenario `specs/agentic-loop/spec.md:176` and normative sentence `:41-44`; test `loop_rebuild_test.go` (`TestARebuiltLoopDoesNotReAskForATurnItCannotRecover`), commit `1709421e` |
| **Owner Codex round 2, finding 3** (2026-09-23) — a first-iteration truncation retry still reads as an untouched birth | **FIXED.** A length-truncated first response self-heals by re-asking the same iteration under the next retry ordinal: it publishes `:req:1:1`, leaves `Iterations` at zero and runs no tool, so the record it leaves is byte-for-byte the shape the republish arm called an untouched birth. Redelivering the original task therefore minted `:req:1:0` under a NEWER retained request and the cold adopt refused the backward name as Fatal — a routine at-least-once redelivery quarantining the `agent.task` lane, which runs at `MaxAckPending` 1, for every task behind it. The republish arm now also requires the record to NAME the loop's first request, which is what the delta's GIVEN has always said; the code did not read it. A record naming anything else — a retry, a later iteration, or nothing — is a loop that moved past its task, and the request it does name is answered on the lane that owns it. No exported surface | `loop_classification.go:226` (the `PublishedRequestID == firstRequest` clause, name minted at `:223`), audit line `component.go:1502`; design § 5.1 steps 3-4, § 1 Q1 row, § 3 item 6; delta scenario `specs/agentic-loop/spec.md:132` unchanged — it already stated the condition; test `task_redelivery_integration_test.go:489` (`TestATaskRedeliveredAfterItsFirstIterationRetriedIsNotRepublished`), commit `122606ed` |
| **Owner Codex round 2, finding 4** (2026-09-23) — cold R1 reconstruction refreshed the original deadline | **FIXED.** This is the only reconstruction that runs the ORDINARY `HandleTask`, whose `configureLoopMetadata` calls `SetTimeout` (`state.go:1372`), which stamps `StartedAt = now` and `TimeoutAt = now + budget`; only the revision was restored afterwards. An expired record whose task redelivered before its response therefore resumed on a full fresh budget — a loop outliving the budget its caller set, and a deviation from the owner's explicit ruling ([issuecomment-5781101792](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5781101792)), blocking at any label. The arm now overlays the record's two timing fields through the existing `GetLoop`/`UpdateLoop` seam; a record with no deadline overlays zero onto zero, which is what the wholesale seat gives, and a failure of either call refuses the delivery rather than republishing on a forbidden deadline. **Class sweep:** the only writers of either field in production code are `SetTimeout`, reached only from `configureLoopMetadata`, reached from `HandleTask` on three paths — birth (intended), this arm (fixed), and a WARM continuation (`handlers.go:932` runs for `continuation == true`), which refreshes a live loop's deadline and is pre-existing on `main`, **recorded as a residual, not built and not filed**. **Second residual:** the R1 arm republishes `agent.created` on every redelivery — the loop-created event carries no `Nats-Msg-Id`, so the server cannot collapse it — recorded in the migration note beside the `tasks_submitted_total` at-least-once rule. **Third residual:** this arm drops a `PendingContinuation` marker without the warning the ruled clear-on-rebuild gives, because it builds a fresh entity rather than seating the record; the turn is lost either way and the ruled limitation sentence already covers the loss. No exported surface | overlay `component.go:1660` calling `state.go:1404` (`LoopManager.restoreDeadline`, which replaced the component-level `restoreRecordedLoopDeadline` in the in-house sweep before round 3 — see the M4 row below); delta scenario `specs/agentic-loop/spec.md:83` (the existing one at `:74` is scoped by its WHEN to an input naming the request, and a task names the loop); migration `docs/operations/migration-beta162-to-beta163.md:1857`; test `task_redelivery_integration_test.go:595` (`TestAColdR1ReconstructionKeepsTheRecordsDeadline`), commit `73370641` |
| **Owner Codex round 2, finding 1** (2026-09-23) — a new cold continuation is acknowledged as an already-applied task | **FIXED as ruled (REFUSE).** `classifyRedeliveredTask` received only the loop ID, so a task naming a loop whose record belongs to a DIFFERENT task was answered from the record alone — and a record can only answer for the task it is about. Advanced, it acknowledged the turn as applied and the user's text was never sent to anyone; at iteration zero with an empty set it took the republish arm, where `HandleTask` cold-created the loop from the ARRIVING task, seating the whole conversation from the new turn's prompt and overwriting the record's `task_id` on the next write. The classifier now takes the task and compares `TaskID` before either arm, returning a fourth disposition; the turn is acknowledged WITHOUT EFFECT after a `WarnContext` naming both tasks and a `continuation_unheld` reason on the existing `task_intake_rejections_total` (lane `cold-fork`) — the settlement the warm `ErrLoopBusy` refusal already takes. Retry would park the `MaxAckPending`-1 task lane on a message no redelivery can fix; Quarantine would latch the lane and health over a valid turn; restore-and-attach was rejected because the record carries no "batch outstanding" fact. **Owner ruling 2026-09-23** ([issuecomment-5791390564](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5791390564)), verbatim "agree on both - continue". **Class sweep:** the two live arms are the finding and share the one fix; the stale+terminal arm checks `State` only and is deliberately unchanged, because a continuation of a settled loop settles either way and the warm lane answers it the same (`ErrLoopTerminal`); `refuseConflictingTaskIdentity` covers the INVERSE relation on the warm map and has no cold analogue. **Design correction:** § 5.1 step 2 claimed task/role/model verification that did not exist — it now names where the task check happens and records that role and model are deliberately NOT verified, because a continuation legitimately re-sends them. **Residual RECORDED, not built and not filed:** the caller observes "Task submitted" and then silence, exactly as it does for the warm refusal today; closing that seam is dispatch's and is not L4a's. No exported surface | disposition `loop_classification.go:121`, gate `:188`; refusal `component.go:1429` (`settleUnheldContinuation`) called at `:1483`, labels `:42-43`; metric Help `metrics.go:259` and recorder doc `:428`; `doc.go:285` § Recovery across a process replacement; migration `docs/operations/migration-beta162-to-beta163.md:1785`; delta normative sentence `specs/agentic-loop/spec.md:45-49` and scenario `:158`; design § 5.1 steps 2 and 4; test `task_redelivery_integration_test.go:718` (`TestAColdContinuationForALoopNoProcessHoldsIsRefused`), commit `d0b7aeda` |
| **Owner Codex round 2, finding 5** (2026-09-23) — the generated I2/I4 coverage claimed in task 4.1 is vacuous | **FIXED as ruled (NARROW).** The property's advance action drains the applied set and writes the advanced record with it empty, so the durable set between actions was always empty and the I2 loop never reached an assertion; no action creates an approval gate, so the I4 branch was never true. Both checks are DELETED — a check that cannot fire is a coverage claim written in code — along with the `toolCalls` ledger the I2 check was the only reader of. The test header, task 4.1 and § 4's PBT decision now claim what the property generates: I1, I3 in its derived form, and "no request is published twice while the stream holds it". I2 cites five named examples; I4 cites the step-0 adopt's two. The generated apply/gate steps were NOT built. **Owner ruling 2026-09-23**, same comment. **Premise correction, measured:** the docket recommended the new I4 example on the ground that the step-0 gate clear was untested — it was not. `TestColdReadAdoptsTheNewestRetainedRequestFirst` already carries "a pending approval gate is cleared in the same write", and the finding-5 mutant reds BOTH tests; the docket's search enumerated a fixed file list that excluded `loop_carrier_test.go`. The ruled example was landed rather than silently dropped, and what it adds is the record the adopt RETURNS — which the classification running next reads and the durable check cannot see — plus a gate built by the production constructor. **Q9 RULED 2026-09-23 "keep both"** (#1330 issuecomment-5792236322): both examples stay. No production code | header `applied_facts_property_test.go:75-111`, deleted checks now `:306-309`; task 4.1; design § 4 PBT decision `:211`; new example `loop_record_writer_test.go:203` (`TestAnAdoptedRequestClearsTheGateItAdvancedPast`), existing arm `loop_carrier_test.go:537`, commit `a3de104c` |
| **Owner Codex round 2, finding 2** (2026-09-23) — approval-gate creation took the publication order deferred to L4b | **FIXED.** An `awaiting_approval` result is not terminal, so the carrier's non-terminal branch took the tool-result lane's `publishThenWrite` and published the `ApprovalPendingEvent` before writing the gate. A crash between them leaves a human an approval request with no durable gate, and the replacement's approval-response handler stale-drops the answer and acknowledges it; the branch that would recover it is the approval lane's cold arm, which is #1362's. § 5.4's "the gate keeps write → publish and there is no W4 here" was false as shipped and is true now. The carve-out is one clause in the CARRIER rather than at the call site, because the `carrierOrder` contract is where the reader looks for which result takes which order and the tool lane asks for publish-first for everything else it produces. **Hole class:** `checkApprovalGate` is the only producer of an `awaiting_approval` result in production code and is reachable only from `HandleToolResult`; the response lane cannot produce one. **Interaction with the stamp reorder (finding 3 of round 1):** a gate result mints no request — its only publication is the `ApprovalPendingEvent`, built with no `MsgID` — so `mintedRequestID` returns `""` for it and `stampPublishedRequest` is a no-op on this path under either order, which is stated in the carrier's doc. No exported surface | clause `component.go:2288` (`gated` at `:2291`), order contract `component.go:2269-2286` and the `carrierOrder` const doc `component.go:2227-2249`; delta carve-out `specs/agentic-loop/spec.md:28-30`; test `loop_carrier_test.go:149` (`TestAnApprovalGateIsWrittenBeforeItsEventIsPublished`), commit `c0a31dff` |
| **In-house sweep before round 3, B1** (2026-09-23) — the approval-timeout sweeper published a minted request nothing stamped | **FIXED.** The sweeper is the one publisher that is not a `persistHandlerResult` call site, so round 2's carrier stamp never ran on it, and its auto-reject mints: the record it wrote moved `iterations` without moving `published_request_id` (I3 violated, nothing repairs it) and every response to the request it did publish was refused as not-yet-observable to `MaxDeliver`. `stampPublishedRequest` now sits between the sweeper's publish and its write, log-only like its neighbours, keeping the publish-then-write order #1362 owns. Round 3 finding 1 ([issuecomment-5792346704](https://github.com/C360Studio/semstreams/issues/1330#issuecomment-5792346704)) is the same defect, found independently | `approval_sweeper.go:146`, comment `approval_sweeper.go:104`; test `loop_carrier_test.go:223` (`TestTheApprovalTimeoutSweepNamesTheRequestItPublished`), whose second arm carries the named request through to an applied response at `:310`; commit `6d445c64` |
| **In-house sweep before round 3, M4** (2026-09-23) — the rebuilt loop's deadline was restored by whole-entity read-modify-write | **FIXED (consolidation).** `restoreRecordedLoopDeadline` read the entity with `GetLoop`, set `StartedAt`/`TimeoutAt` on the copy and wrote it back with `UpdateLoop`, which replaces the whole entity — so a mutation a sibling lane committed between the two lock acquisitions was discarded. Reachable: by the time the cold R1 arm runs it the loop is registered and R1 tracked, so the response lane, a separate consumer, can be applying the predecessor's answer. A lost update, not a data race, so `-race` is blind to it. The existing owner of both fields is `SetTimeout`, in place under the manager lock; `restoreDeadline` joins it there and the read-modify-write is deleted | `state.go:1404` beside `SetTimeout` (`state.go:1372`), call site `component.go:1660`; test `loop_rebuild_test.go:502` (`TestRestoringARecordsDeadlineTouchesOnlyTheTwoFieldsItOwns`) plus the existing `TestAColdR1ReconstructionKeepsTheRecordsDeadline`; commit `d227b2cd` |
| **In-house sweep before round 3, M5** (2026-09-23) — the rebuild's dropped-turn warning had no observer | **FIXED (test) and RECORDED (site).** The delta's THEN promises the marker is cleared AND a warning names the loop; the test asserted only the clear, so the one signal that an accepted turn was dropped was unobserved. The rebuild test now takes the manager's own logger and reads the warning off its own line — other warning-level lines carry a `loop_id`, so a buffer-wide check was satisfiable without it. No counter was added: the ruled shape of this clear is the two-line clear plus a warning, and a counter here is an owner question. Round 3 finding 4 asks for exactly this assertion | warning `state.go:417`, its rationale `state.go:410`; test `loop_rebuild_test.go:451` and `:452`; commits `65d9db8f`, `3869ab17` |
| **Owner Codex round 3, finding 3 / Q11** (2026-09-23, ruled "yes") — an R1 replay seats a fresh loop over a dispatched batch | **FIXED as ruled.** A handled tool-call response for R1 leaves the record at `{R1, iterations 0, empty applied set}` — the republish arm's exact shape — while the batch it dispatched is outstanding in a process that is gone, so the arm seated a fresh loop with no batch: the first tool result had no execution to route to, its cold rebuild was refused over the seat, and nothing released it. The arm now requires the fourth fact, which is on the STREAM and not on the record: nothing retained for the loop, which is the crash window between the record write and the publish. Anything retained — answered or not — is `taskApplied`: acknowledged, no seat, no publish, and the loop is rebuilt by the lane that owns its outstanding work, its own response or its first tool result. The read is `readRetainedAgentRequest`, the one the publish path would have paid, taken before the loop is seated; a transient read failure returns transient as the record read above it does, and a fatal one stays fatal. The three record facts keep their jobs, stated at the site. **Bonus closed:** a record naming `:req:1:0` under a newest-retained `:req:1:1` — the retry W4 on the task lane — now ACKs instead of minting `:req:1:0` into the cold adopt's Fatal backward-name branch. Q1/Q5's "the publish path adopts a retained R1" is superseded ON THIS LANE (see the Q1 DEVIATION row); its substance stands. No exported surface | read `loop_classification.go:244`, republish only when nothing is found `:254`, arm comment `:193-243`, reader `loop_evidence.go:156`, republish arm `component.go:1631`; delta scenarios `specs/agentic-loop/spec.md:135` and `:143`; integration `task_redelivery_integration_test.go:271` (`TestATaskRedeliveredOverADispatchedFirstBatchIsAcknowledged`), the W2 fixture the republish arm now needs `:41` (`birthWhosePublishNeverLanded`), unit `recovery_test.go:867`; commit `cdc2045e` |
| **Owner Codex round 3, finding 2** (2026-09-23, docket option (b); **Q10 unanswered**, so the admission write and the ruled Q2 rebuild warning both stay) — a sibling write serialises the unpublished advance | **FIXED.** Round 1 moved the request-ID stamp to the carrier and closed half the window. The other half: by the time a continuation can DEFER, `TrackRequest` has run, so `handleToolsComplete` has already incremented `Iterations` and drained `PendingToolResults` on the shared entity behind a request that is still unpublished — and every record write in this process RENDERS that entity, so the sibling committed `{R1, N+1, empty}`. `iterations` moved in an update whose `published_request_id` did not (I3), and a crash there leaves a record claiming a batch was consumed by a request the stream never received: replaying the batch's last result reads the drained set as never applied, re-queues every sibling for re-execution, increments again and skips an ordinal. The deferred lane no longer renders the live entity: it reads the RECORD under `loopRecordMu`, overlays only `PendingContinuation`/`PendingContinuationRequestID`, and compare-and-swaps against the revision this process observed — keeping OQ3's fence, so a foreign advance still refuses and the turn is redelivered. An absent or terminal record is a declared skip with a warning. The advance rides the carrier's own write, after the PubAck. **Rejected options, recorded:** (a) one region per loop stalls every loop's record write behind one PubAck and ratchets complexity; (c) drops the ruled Q2 warning and the admission fence and needs a re-rule; (d) has no snapshot source to render from. No exported surface | `component.go:3255` (`persistDeferredContinuationMarker`), overlay `:3288` and `:3292`, CAS `:3298`, call site `component.go:1537`; design § 3 item 4; delta scenario `specs/agentic-loop/spec.md:180`; test `deferred_continuation_record_integration_test.go:178` (`TestADeferredContinuationWritesOnlyTheMarkerItOwns`), its interval fixture `:39` (`startMidAdvance`); residual § 7 item 15; commit `c7a333d9` |
| **Q12** (2026-09-23, ruled "yes") — a redelivered CURRENT model response is re-applied to a warm loop | **FIXED as ruled (option (a)).** Ordering answers three of the four cases; the fourth — a second delivery of the current request's answer — was handled again from the top, because both deliveries name the request the record names. Tool EXECUTION survives that (deterministic identity, `TOOL_CALL_OUTCOMES` replays the outcome), the loop's CONVERSATION does not: the assistant turn is appended again and rides the next request the model is asked to answer, and on a gated loop the re-dispatch runs while a human is being asked to approve one of those calls. The separating fact is the outstanding mark, which `SettleRequest` clears the moment an answer is used: `requestOrderCurrent` on a loop waiting on no request is acknowledged without effect, logged and counted. **Premise inventory, before code:** (1) `SettleRequest` clears the mark only for the request named, so "current and not outstanding" on a loop this process holds means applied HERE; (2) the cold arm needed NO change — `restoreLoopFromRequest` already marks the retained request outstanding (`state.go:490`), so a just-rebuilt loop's first response passes the gate, which is fewer lines than either alternative the ruling offered; (3) the arm sits AFTER the order switch and is scoped to `requestOrderCurrent`, so older, newer, foreign and `requestOrderUnnamed` keep their dispositions. **Counter:** the response lane's existing `model_responses_dropped_total` gains the reason value `already_applied` — the one the tool lane already uses for the same shape — no new metric family. **Two fixtures corrected with it,** both asserting states production cannot produce: a replacement that HOLDS a loop holds its outstanding request too (every seat sets both), and the terminal guard is reached by a response the loop is still waiting on after a cancel, not by one it had already answered. No exported surface | guard `handlers.go:1393`, sentinel `handlers.go:1282`, settlement `component.go:1880`, metric help `metrics.go:180`; `doc.go:247`; migration `docs/operations/migration-beta162-to-beta163.md:1178`; delta scenario `specs/agentic-loop/spec.md:115`; design § 5.2 windows line; test `response_redelivery_integration_test.go:44` (`TestARedeliveredResponseTheLoopAlreadyAppliedIsAcknowledged`); fixtures `settlement_recovery_test.go:33`, `handlers_test.go:1250`; commit `0bbee770` |
| **Owner Codex round 4, finding 1** (2026-09-23) — a failed publication still advanced the durable request identity | **FIXED.** The approval-timeout sweeper is the one publisher that is not a `persistHandlerResult` call site, and its auto-reject MINTS (`handleRejectedApproval` → `HandleToolResult` → `handleToolsComplete`). Round 3 gave it the carrier's stamp; the stamp and the record write both still ran after a publication that failed, so with KV writable the record named `R2` while the stream retained only `R1` — I1 broken on a loop nothing repairs, and every later cold read took `adoptNewerRetainedRequest`'s Fatal I1 arm over an approval that merely timed out. A failed publication now stops the pair: neither the stamp nor the persist runs and the record keeps the gated predecessor state a replacement can still recover from. Skipping only the stamp was refused explicitly — persisting the advanced entity under the old name is the iteration/identity mismatch the stamp exists to prevent. The in-memory advance is left as it is and named at the site: a timer has no delivery to classify, and the sweeper's retry/counter policy travels with the lane to #1362 (§ 5.6). **Sweep, one path over:** the four production sites that publish and then stamp or persist were enumerated — the carrier's two branches (`component.go:2341` write-then-publish, publish last and nothing follows it; `component.go:2363` publish-then-write, which returns before the stamp on this exact error), birth (`component.go:1714`, which releases and returns), and the sweeper — plus the two terminal writers that publish after their write and mint nothing (`component.go:2060` failure, `component.go:3469` cancel). One hit: the sweeper | `approval_sweeper.go:139` (the `continue`), stamp `approval_sweeper.go:146`, persist `:152`; carrier's shape `component.go:2363`; delta scenario `specs/agentic-loop/spec.md:103`; § 5.6; test `approval_timeout_publish_failure_integration_test.go:48` (`TestAnApprovalTimeoutWhosePublicationFailedLeavesTheRecordAsItWas`); commit `9b641bc7` |
| **Owner Codex round 4, finding 2** (2026-09-23) — the Q12 replay warning had no observer | **FIXED (test).** Q12's regression observed the counter, the single dispatch, the single assistant turn and the unmoved revision, every one of which survives deleting the guard's `Warn`; the recorded guard mutant fails on the duplicate dispatch, so it established nothing about the warning while the new scenario promises an audit line naming the loop and the request. The handler's logger is captured for the replay delivery only and the assertion reads the one LINE carrying the message — the lane warns on superseded and foreign responses too, and both carry a `loop_id` | warning `handlers.go:1394`; observer `response_redelivery_integration_test.go:125` in `TestARedeliveredResponseTheLoopAlreadyAppliedIsAcknowledged`; helper `loop_rebuild_test.go:535` (`logLineContaining`); commit `15cf51b5` |
| **Owner Codex round 4, finding 3** (2026-09-23) — three surviving comments still stated the pre-Q11/Q12 rule | **FIXED (documentation).** The republish disposition described a birth that published `R1` and died before acknowledging, which Q11 moved to `taskApplied`; the task lane's cold-fork test said the arm runs with "no retained read to decide it"; and `OutstandingRequest`'s doc said a redelivery arriving after the mark was cleared "must still be handled". **Sweep, one path over** (`git grep -n` for "died before Ack", "no retained read", "outstanding mark", "must still be handled" across `processor/agentic-loop/` and the change docs, plus the tree's own wording "died before acknowledging"): two further code hits — `HandleModelResponse`'s ordering comment promising a redelivery of the current request "is still handled", and `TestSettleRequestOnlyClearsTheRequestItNames` opening by calling the mark the authority on superseded responses — and three change-doc hits, marked superseded in place rather than rewritten, since `tasks.md`'s task descriptions are baseline evidence. Quotations of the Q1 ruling's own words (`tasks.md:505`, `:517`) stand: they are what the ruling said | `loop_classification.go:94-101` and `:103-107`, `recovery_test.go:795-799`, `state.go:1186-1200`, `handlers.go:1327-1330`, `outstanding_request_test.go:9-13`; markers `tasks.md` 2.1 and 3.4; commit `12ccb48f` |
