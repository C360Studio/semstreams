...
- [ ] 1.1 In `persistHandlerResult` — `processor/agentic-loop/component.go:2218` — `func (c *Component) persistHandlerResult(ctx context.Context, result HandlerResult) error {` —
      ...
      is NOT widened (design P1). Doc comment names #1377 W3/W4 and the publication in flight.
- [ ] 1.2 Reword `settleTerminalGuard`'s Warn and transient cause —
      ...
      decides", logging the record's state beside the result's (design P7).
- [ ] 1.3 Doc comments on `recordTerminalToolResultDropped` (TO:506-511) and on the metric help —
      ...
      auto-reject included), while the handler-entry guards keep their own families (design § 7).

## 2. The carrier's own write refuses a terminal snapshot (design § 3.2; OQ3 (ii))

- [ ] 2.1 **[OQ3 (ii)]** Rename the body of `processor/agentic-loop/component.go:3025` — `func (c *Component) persistLoopState(ctx context.Context, loopID string) error {`
...
- [ ] 3.1 Add `testApprovedDispatchHook func(loopID, stage string)` to `MessageHandler` and
      `testCarrierHook func(loopID, stage string)` to `Component` (unexported; doc comments cite the precedent
      `processor/agentic-loop/component.go:157` — `testPublishHook func(subject string, data []byte)`).
- [ ] 3.2 Call sites, nil-checked: `before_dispatch` after `processor/agentic-loop/approval_response_handler.go:103` — `}` (the `IsTimedOut` block);
      ...
      unforced and 4.3 is marked unproven.
...
- [ ] 5.1 T3/T4: move the probe's harness (`raceProbeBucket` `processor/agentic-loop/held_loop_cancel_approval_race_probe_integration_test.go:84` — `type raceProbeBucket struct {`,
...
- [ ] 5.3 T1: the two-component W1 counterexample on the `predecessor`/replacement pattern —
      ...
      else name it unproven (design § 10). Asserts the documented bound; no mutation (an (a) row).
- [ ] 5.4 T2: extend `processor/agentic-loop/approval_loop_deadline_test.go:219` — `func TestASweepTimeoutWhosePublishFailedSettlesOnTheNextAnswer(t *testing.T) {`
      ...
      **[OQ2 (b)]** assert nothing dispatched, `approval_inapplicable` +1, record `failed` with the saved reason, and
      mutation evidence: delete the marker read of 4.1 → red on "no tool.execute".
- [ ] 5.5 Fix the one test the check changes: `processor/agentic-loop/publish_phase_fatal_test.go:31` — `t.Parallel()` — `TestPublishPhaseFailureLeavesPersistHandlerResultFatalClassified`
...
- [ ] 5.6 Mutation evidence for the WIRING (`cp` backup, `md5 -q` before and after equal): delete the 1.1 block → T3
      ...
      T9 red on the write count. Record the runs in the PR body. **[OQ2 (b)]**: the 5.4 mutation.
- [ ] 5.7 Controls green by name with `-count=1`, output pasted — the 14 order tests the archived transition-result
      ...
      S:886 "recovered cold" scenario's test(s) are re-read for the first-delivery cold branch.
- [ ] 5.8 `openspec validate agentic-loop-committed-terminal-recovery --strict` green; `task spec:properties` count
      moves by the new `// spec:` citations (`git add` the new test file first).
- [ ] 5.9 Migration: replace `docs/operations/migration-beta162-to-beta163.md:2015` — `**Two residuals, recorded and not reconciled** (#1362 issuecomment-5808903072 and issuecomment-5809906669):`
      ...
      the spec's residual sentences are replaced through the delta's sync (6.5).
