...
- [x] 1.2 Verify this delta strands no `// spec:` citation: the restated requirement heading is byte-identical,
  ...
  **3 of 48 citations unresolved**, all in a different capability (`agentic-loop`) and none introduced here.
  ...
  and `:492`, `terminal_release_test.go:437`, all citing *Audit loss degrades loudly and never fails agent work*.
...
- [x] 2.5 Record the ruling-to-file conformance table in `proposal.md`, covering all nine Tier 1 removals and
  the semsage obligation. It surfaced that `LoopEntity.StateBeforePause` is named by neither ruling
