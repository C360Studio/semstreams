...
- The authority-pair budget tightens: 168 bytes effective, 161 declared (the agent-run family now binds).
...
loop UUID. Those two classes are obligations 1 and 2 below — that silence is why this section exists.
...
| TS slicing the bare id | semteams `ui/src/lib/stores/runStatus.svelte.ts:52,171`; `ui/src/lib/utils/runHealth.ts:136,151-154`; the `ui/e2e/agentic/*.spec.ts` fixtures | **silently** — the slice returns a 64-hex digest, not a loop id | read `run_entity_id` from the wire; resolve the origin loop via `agent.run.origin-entity-id` |
