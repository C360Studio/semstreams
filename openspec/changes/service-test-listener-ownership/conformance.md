...
| Preserve public port-zero meanings and native occupied-port refusal | `metric/handler.go:42`, `service/service_manager.go:1284`; refusal tests `service/service_manager_health_listener_test.go:183` and `service/startup_observability_test.go:660`. |
| Keep native metric Start acquisition/request/Stop positive proof | `metric/handler_test.go:93` starts the real server with its private port set to zero, requests Address, and stops it. |
| Add only StartWithListener; successful return transfers the original raw TCP listener | `metric/handler.go:66`; original-listener request/accept/close observations at `metric/handler_test.go:114` and `:154`. |
| Every synchronous rejection leaves supplied listener caller-owned; preserve one-shot ordering | `metric/handler.go:70`; refusal, preparation failure, and used-instance tests at `metric/handler_test.go:167`, `:206`, and `:229`. |
| Address observes actual owned endpoint, retains scheme/path, uses configured fallback without ownership | `metric/handler.go:272`; mismatch/fallback tests at `metric/handler_test.go:114` and TLS scheme/path proof at `:249`. |
| Rejected second Start preserves the first owned endpoint | `metric/handler_test.go:229` checks original address and both listener ownership states after rejection. |
| TLS wraps exactly once; request contexts descend from caller Start | `metric/handler_test.go:114` checks exact BaseContext; `:226` performs a real HTTPS request with the generated test CA. |
...
| Retention, shutdown, Address, and both service bridge mutations detect the named violations | Five detected mutants across four required families have baseline/mutant/restored exits 0/1/0; final source/test hashes and assertion output are retained in `evidence.md`. |
