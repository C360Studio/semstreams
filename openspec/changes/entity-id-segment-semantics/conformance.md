...
| D5 | Subject-only coded authority rejection; import lane; `@id` objects structural only (no stub, absent object permitted); foreign-subject mutation per O-12 (d5) | | ADDED "Authority mismatch is a coded rejection distinct from structural rejection"; graph-ingest ADDED | `TestAuthorityRejectionIsCodedAndIdentityFree`, `TestAuthorityRejectionLocalClaimOnImportLane` |
...
| O-12 | Mirror vs annotate for foreign-authority subjects (owner; pre-landing — gates `Closes #1096` for imported firing entities) | | graph-ingest ADDED requirement's O-12 clause resolves | ruling comment URL; `TestRunScopeNewMintsUnderDeploymentAuthority` under the ruled option |
