# Conformance — entity-id-segment-semantics (skeleton, revision 3)

Per-ruling map from the owner rulings on #1095 (2026-08-26), the two design constraints, and ADR-102 decisions to
...
| R4 | Rule read-back is a bug (#1096) | | `specs/graph-ingest/spec.md` ADDED "Framework-minted runtime state carries the deployment's own authority" | `TestRunScopeNewMintsUnderDeploymentAuthority` |
...
| C1 | Reordering allowed (constraint) | | `specs/entity-id-contract/spec.md` MODIFIED "Every entity ID has one canonical six-segment ASCII form" | `TestEntityIDKeyOrderIsSystemBeforeDomain` |
| C2 | Arity stays six (constraint) | | same (unchanged arity clause) | existing `pkg/types` arity tests |
| C3 | Second- and third-order impacts of the six-part shape made explicit (constraint) | `docs/proposals/gh1095-entity-id-segment-semantics-inventory.md` §1 (rows S/K/X/W/C/H/L/R/M/D/F/T/P + census) | — | independent inventory pass (PASS WITH DIVERGENCES, r2 corrections) |
...
| D5 | Subject-only coded authority rejection; import lane; `@id` objects structural + must-exist; foreign-subject mutation per O-12 (d5) | | ADDED "Authority mismatch is a coded rejection distinct from structural rejection"; graph-ingest ADDED | `TestAuthorityRejectionIsCodedAndIdentityFree`, `TestAuthorityRejectionLocalClaimOnImportLane` |
...
| H | Hierarchy inference skips foreign-authority entities (ADR-102 consequence; O-6 pending) | | `specs/graph-ingest/spec.md` ADDED "Hierarchy inference skips foreign-authority entities" | `TestHierarchySkipsForeignAuthority` |
...
| W9 | `iot_sensor` zone reader reads positions by name | | — | `TestParseZoneEntityIDReadsNamedPositions` |
...
| O-11 | Served ADR-099 level after the reorder (owner) | applied in gh606, not here | — | ruling comment URL |
| O-12 | Mirror vs annotate for foreign-authority subjects (owner) | | graph-ingest ADDED requirement's O-12 clause resolves | ruling comment URL |
| O-13 | Imported lessons' local applicability (owner) | | — | ruling comment URL |
| O-14 | Authority-pair bound at config load (owner, decide-from-evidence) | | ADDED "The authority pair is bounded at configuration load" | `TestConfigRejectsOversizedAuthorityPair` |
