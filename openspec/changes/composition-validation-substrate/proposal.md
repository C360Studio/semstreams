...
- **Removal (last).** `flowstore/`, `flowtemplate/`, `engine/`, `service/flow_service.go`,
  ...
  before `FlowService` is deleted (`service/flow_service.go:560-585` is its only host).
...
- **BREAKING**: semstreams-ui (`/flowbuilder/*` at 15 call sites in 8 files; its e2e global setup reaps flows through
  ...
  MCP validate), generated TS types in semspec/semdragon (residue). Migration document
  `docs/operations/migration-composition-validation-adr100.md` is part of the removal commit.
- **E2E before the removal commit lands**: `task e2e:core` (boot path; every tiered `Setup` calls
  `<components>/validate`, `test/e2e/scenarios/tiered.go:187`), `task e2e:crud-tools` (tool registry boots without
  the flow gates), `task e2e:agentic` (largest shipped composition through the P5 boot check).
