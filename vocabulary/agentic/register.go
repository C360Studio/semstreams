...
func registerLoopPredicates() {
	...
	vocabulary.Register(LoopTerminalReason,
		vocabulary.WithDescription("Classified reason for a failed loop's terminal outcome (max_iterations, model_error, length_truncated, handler_error, timeout, continuation_unavailable, spawn_identity_birth_failed, graph_state_reset_required, record_exceeds_payload_ceiling); absent on success"),
		vocabulary.WithDataType(vocabulary.DataTypeString))
