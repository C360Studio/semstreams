...
//   - Stale owning-lease compaction via a separate OWNER_PRESENCE heartbeat
//     bucket (liveness over a dead owner's replace/CAS claims — the same call
//     NATS KV TTLs make), and OwnerOf, the write-time lease lookup the
//     graph-ingest mutation handlers consult. Non-owning append/foreign-edge
//     registrations persist without presence state.
//
// W0 first consumer (landed — Decision 5 embed):
//
//   - EnsureBuckets creates the two framework buckets (OWNER_CLAIMS with audit
//     history; OWNER_PRESENCE with the PresenceTTL backstop for owning leases)
//     and returns a Registry. It is called EAGERLY in the framework boot path,
//     before lifecycle registration — NOT in graph-ingest, which boots after.
//   - Heartbeater is the substrate-owned owning-lease ticker an embedder drives
//     over its app-root context (no Close to adopt; ctx cancellation stops it).
...
//   - Registrations containing only append-mode owner claims and/or foreign-edge
//     claims are durable declarations, not leases: they receive no presence key,
//     are not heartbeat- or revival-monitored, and persist through compaction.
//     If the same atomic registration also contains a replace/CAS claim, the
//     whole entry is an owning lease: presence and liveness apply to the entry,
//     and stale compaction removes all of its claims together.
...
//     write commits); PR-3.5 typed opaque OwnerToken for owning registrations
//     (non-owning Bind returns the zero token); PR-4 post-registration
//     Watch-revival quiesce for owning leases (WatchRevival quiesces the
