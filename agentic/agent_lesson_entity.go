...
func (e *AgentLessonEntity) EntityID() string {
	...
	if err != nil {
		// entity-id-audit:classify intentional-sentinel "" line=222 column=10 surface=go-return:EntityID entity_id_invalid:empty documented lesson failure return; graph-ingest rejects an empty ID and a decoded payload must not panic
		return ""
	}
	return id
}
...
func LessonContract() contract.Contract {
	return contract.Contract{
		...
		EntityPattern: "*.*.lesson.agent.record.*",
...
//
// Format: {org}.{platform}.lesson.agent.record.{id}
//
// Example: AgentLessonEntityID("acme", "ops", "2c5acb9b-8283-5b34-a4d1-4b1c9f8502ca")
// Returns: "acme.ops.lesson.agent.record.2c5acb9b-8283-5b34-a4d1-4b1c9f8502ca"
//
...
func tryAgentLessonEntityID(org, platform, id string) (string, error) {
	...
	entityID := fmt.Sprintf("%s.%s.lesson.agent.record.%s", org, platform, id)
...
// lesson record for an org/platform: "{org}.{platform}.lesson.agent.record".
//
...
// "lesson.agent.record" segment string here — beside AgentLessonEntityID —
...
func AgentLessonRecordPrefix(org, platform string) string {
	...
	return fmt.Sprintf("%s.%s.lesson.agent.record", org, platform)
}
