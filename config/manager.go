...
//
// Events with revision <= engineHighWaterRev are skipped: those
// events were generated by the Manager's own write methods
// (PutComponentToKV, DeleteComponentFromKV, PushToKV) which already
// applied the change to in-memory state synchronously. Re-applying
// from the watcher would (a) be redundant and (b) can overwrite more
// recent engine writes when the watcher is lagging behind a burst
// of operations (the canonical Stop-then-Undeploy race surfaced by
// TestEngineIntegrationSuite/TestFullLifecycle).
//
// External writes (UI, other processes) produce revisions strictly
// greater than the engine's watermark at the time they wrote, so
// they apply normally. Subscribers are notified for both engine and
// external events — the skip only affects the in-memory cache
// update, not subscriber notification.
func (cm *Manager) handleUpdate(key string, value []byte, revision uint64) {
	...
	// Skip in-memory apply for events produced by our own writes.
	// The engine wrote these synchronously to memory; re-applying
	// from the watcher's queue can override more recent engine state.
	if revision != 0 && revision <= cm.engineHighWaterRev.Load() {
		cm.logger.Debug("Skipping watcher event for engine-owned revision",
			"key", key,
			"revision", revision,
...
//
// The Delete generates a watcher event at a fresh revision but the
// underlying API doesn't expose that revision back. The watermark
// tracking handles this correctly without capturing the delete's
// revision: the watcher event arrives with rev > engineHighWaterRev
// and applies a delete-on-already-absent key (the engine already
// removed it from memory synchronously), which is an idempotent
// no-op. Stale PRE-delete PUT events for the same key (e.g. a
...
//
// The revision returned by KV.Put is captured into engineHighWaterRev
// so the watcher's handleUpdate skips the resulting event — the
