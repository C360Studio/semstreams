...
// RegisterTool opens (or creates) the AGENT_LOOPS bucket and wires the
// research_graph tool. Bucket config matches
...
//
// Unlike optional core tools, a selected graph-research capability fails boot
// when its store or result-reading path is unavailable. Advertising only part
// of the asynchronous chain would strand parent loops.
//
...
// Callers must run ValidateConfig before invoking this function.
func RegisterTool(ctx context.Context, tools *agentictools.ExecutorRegistry, natsClient *natsclient.Client, platform component.PlatformMeta, logger *slog.Logger, bucketName string) error {
	if tools == nil {
		return fmt.Errorf("register graph research tool: nil tool registry")
	}
	if natsClient == nil {
		return fmt.Errorf("register graph research tool: NATS client is required")
	}
	if tools.GetTool(agentictools.ReadLoopResultToolName) == nil {
		return fmt.Errorf("register graph research tool: required result path %q is unavailable", agentictools.ReadLoopResultToolName)
	}
	if logger == nil {
		logger = slog.Default()
	}
	if bucketName == "" {
		bucketName = "AGENT_LOOPS"
	}
	bucket, err := retry.DoWithResult(ctx, retry.Quick(), func() (jetstream.KeyValue, error) {
		return natsClient.CreateKeyValueBucket(ctx, loopResultBucketConfig(bucketName))
	})
	if err != nil {
		return fmt.Errorf("register graph research tool: open loops bucket %q: %w", bucketName, err)
	}
...
func loopResultBucketConfig(bucket string) jetstream.KeyValueConfig {
	return jetstream.KeyValueConfig{Bucket: bucket, History: 10, TTL: 24 * time.Hour}
}
