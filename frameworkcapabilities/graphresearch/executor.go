...
func (e *ResearchGraphExecutor) researchGraph(ctx context.Context, call agentic.ToolCall) (agentic.ToolResult, error) {
	...
	// LoopExecution typed-origin envelope via entity.create. graph-ingest append
	// is must-exist, so a kickoff append to an absent entity returns
	// entity_not_found and the chain stalls before R0 fires. Downstream stage
	// stamps append onto the entity born here.
