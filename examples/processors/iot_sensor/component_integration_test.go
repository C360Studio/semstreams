...
func TestIntegrationIoTSensorConsumerReplaysMessagePublishedBeforeStart(t *testing.T) {
	const (
		inputStream   = "GH963_IOT_INPUT"
		inputSubject  = "gh963.sensor.raw"
		inputFilter   = "gh963.sensor.>"
		outputStream  = "GH963_IOT_OUTPUT"
		outputSubject = "gh963.processed.sensor"
		deviceID      = "gh963-sensor-before-start"
	)

	tc := natsclient.NewTestClient(t, natsclient.WithStreams(
		natsclient.TestStreamConfig{Name: inputStream, Subjects: []string{inputFilter}},
	...
	observer, err := output.CreateOrUpdateConsumer(ctx, jetstream.ConsumerConfig{
		Durable:       "gh963-iot-output-observer",
	...
	raw := []byte(`{"device_id":"` + deviceID +
		`","type":"temperature","reading":21.5,"unit":"celsius","location":"lab"}`)
		...
		Ports: &component.PortConfig{
			...
			Outputs: []component.PortDefinition{{
				...
				Config: component.JetStreamPort{
					...
					Interface:  &component.InterfaceContract{Type: "iot.sensor.v1"},
				},
			}},
		},
	})
	...
	for {
		...
		case result := <-delivered:
			...
			if bytes.Contains(result.data, []byte(deviceID)) {
				goto sensorDelivered
			}
		case <-ctx.Done():
			t.Fatalf("pre-start sensor was not processed: %v", ctx.Err())
		}
	}
...
sensorDelivered:
	...
	require.NoError(t, err)
	consumer, err := input.Consumer(ctx, "iot-sensor-gh963-sensor-wildcard")
	require.NoError(t, err)
