	...
	for index := range count {
		definition := component.PortDefinition{
			...
			Config: component.JetStreamPort{
				StreamName: fmt.Sprintf("S1_IOT_%d", index),
				Subjects:   []string{fmt.Sprintf("s1.iot.%d", index)},
			},
		}
...
func TestLifecycleOwnerProductionStartStopOverlapRollsBackExactHandle(t *testing.T) {
	...
	owner := &Component{
		name: "iot-sensor-processor", logger: slog.Default(), natsClient: &natsclient.Client{},
