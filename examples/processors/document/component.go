...
func rejectRemovedConfigKeys(raw json.RawMessage) error {
	...
	for field, guidance := range removedConfigFields {
		if _, found := present[field]; found {
			return errs.WrapInvalid(errs.ErrInvalidConfig, "DocumentComponent", "rejectRemovedConfigKeys",
...
func resolveConfig(rawConfig json.RawMessage) (ComponentConfig, []component.Port, []component.Port, []string, error) {
	...
	if err := json.Unmarshal(rawConfig, &config); err != nil {
		return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(err, "DocumentComponent", "NewComponent", "config unmarshal")
	}
	...
	if len(config.Ports.Outputs) != 1 {
		return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(errs.ErrInvalidConfig, "DocumentComponent", "NewComponent", "exactly one output port is required")
	}
	...
	for _, definition := range config.Ports.Inputs {
		...
		if err != nil {
			return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(err, "DocumentComponent", "NewComponent", "resolve input port")
		}
		facts, err := input.Facts()
		if err != nil || len(facts.NATSSubjects()) != 1 {
			return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(errs.ErrInvalidConfig, "DocumentComponent", "NewComponent", "input port must declare one NATS subject")
		}
	...
	for _, definition := range config.Ports.Outputs {
		...
		if err != nil {
			return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(err, "DocumentComponent", "NewComponent", "resolve output port")
		}
		facts, err := output.Facts()
		if err != nil || len(facts.NATSSubjects()) != 1 {
			return ComponentConfig{}, nil, nil, nil, errs.WrapInvalid(errs.ErrInvalidConfig, "DocumentComponent", "NewComponent", "output port must declare one NATS subject")
		}
