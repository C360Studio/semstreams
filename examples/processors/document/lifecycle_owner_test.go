	...
	for index := range count {
		definition := component.PortDefinition{
			...
			Config: component.JetStreamPort{
				StreamName: fmt.Sprintf("S1_DOCUMENT_%d", index),
				Subjects:   []string{fmt.Sprintf("s1.document.%d", index)},
			},
		}
...
func TestLifecycleOwnerProductionStartStopOverlapRollsBackExactHandle(t *testing.T) {
	...
	owner := &Component{
		name: "document-processor", logger: slog.Default(), natsClient: &natsclient.Client{},
